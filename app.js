...
function updateStaticI18n(){
  ...
  document.querySelectorAll('.bottomnav button').forEach(b=>{const k=b.dataset.view;if(k)b.textContent=t(k);});
}
...
function traceText(x){
  return String(x||'')
    .replace(/source_id\+claim_id確認できる資料/g,'条件と根拠をたどれる資料')
    .replace(/source_id\+claim_id確認済み/g,'条件と根拠をたどれる資料')
...
function publicLabel(x){
  ...
  const map={
    ...
    A:'条件まで見られる資料', B:'要点が読める資料', C:'出典からたどる資料', D:'現場記録'
  };
  if(map[s]) return map[s];
  s=s.replace(/_/g,' ').replace(/\bsource type\b/i,'出典種別').replace(/\bevidence level\b/i,'根拠確認').trim();
  return s;
}
...
function storageStepLabels(){
  return msg(
    ['1. 納米庫で見るべきこと','2. 結露とは何か','3. 湿度管理','4. 温度管理','5. 残米','6. カビ臭・虫・変色','7. 危険サイン','8. 点検場所','9. 今日の補助注意','10. 出典・根拠'],
...
function getWaterLocationHint(){
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    box.innerHTML=`${msg('現在地の緯度経度：','Current latitude/longitude: ','目前緯度經度：','当前位置经纬度：')}${lat}, ${lng}<br>${msg('現在地を取得しました。該当しそうな地域を選ぶと、水質情報の確認先と炊飯で見るポイントを表示します。','Location acquired. Choose the closest area to show water-information links and rice-cooking points.','已取得位置。選擇可能對應的地區後，會顯示水質資訊確認處與炊飯重點。','已获取位置。选择可能对应的地区后，会显示水质信息确认处和煮饭重点。')}`;
...
function explainEvidenceLink(kind='文献'){
  ...
  return msg('詳しい根拠や条件は、文献ライブラリの詳細で確認できます。','Detailed evidence and conditions can be checked in the reference library.','詳細根據與條件可在文獻庫詳情確認。','详细依据与条件可在文献库详情确认。');
}
...
function waterIntroHTML(region,w){
  ...
  return `<p><b>${msg('目的：現在地や選んだ地域から水質の公表情報へ進み、その水で炊く時の見方へ変換します。','Purpose: open public water information for your current or selected area, then translate it into rice-cooking points.','目的：從目前位置或選定地區前往公開水質資訊，並轉換成炊飯時的觀察方式。','目的：从当前位置或选定地区前往公开水质信息，并转换成煮饭时的观察方式。')}</b></p>
  ...
  <details><summary>${msg('pH研究データの読み方','How to read pH research data','pH研究資料的讀法','pH研究资料的读法')}</summary>${phClaims.map(c=>`<p><b>${esc(field(c,'condition')||msg('条件','Condition','條件','条件'))}</b><br>${esc(localizedField(c,'claim')||msg('要点は文献詳細または出典ページで確認します','Check the key point in reference details or on the source page','要點請在文獻詳情或出處頁面確認','要点请在文献详情或出处页面确认'))}<br><span class="small">${msg('注意：','Note: ','注意：','注意：')}${esc(localizedField(c,'display_caution')||msg('飲用水の推奨値ではなく実験条件として扱います。','Treat this as an experimental condition, not a drinking-water recommendation.','請作為實驗條件處理，不是飲用水建議值。','请作为实验条件处理，不是饮用水建议值。'))}</span></p>`).join('')}</details>`;
}
...
function evidenceLabel(c){return {A:msg('条件まで見られる資料','Source with conditions','可確認條件的資料','可确认条件的资料'),B:msg('要点が読める資料','Source with key points','可閱讀要點的資料','可阅读要点的资料'),C:msg('出典からたどる資料','Trace from source','從出處追溯的資料','从出处追溯的资料'),D:msg('現場記録','Field note','現場記錄','现场记录')}[c]||msg('出典からたどる資料','Trace from source','從出處追溯的資料','从出处追溯的资料');}
function statusInfo(c){return {A:msg('資料名、要点、条件がそろっていて、条件まで見られる項目です。','The title, key point, and conditions are available, so conditions can be checked.','資料名、要點與條件齊全，可確認條件。','资料名、要点与条件齐全，可确认条件。'),B:msg('文献や公的資料の要点を読める項目です。','You can read key points from references or public materials.','可閱讀文獻或公開資料的要點。','可阅读文献或公开资料的要点。'),C:msg('タイトルやカテゴリから関連資料を探せる項目です。','Use the title and category to find related source material.','可由標題與分類尋找相關資料。','可由标题与分类寻找相关资料。'),D:msg('文献とは分けて扱う現場記録です。','This is a field note kept separate from references.','這是與文獻分開處理的現場記錄。','这是与文献分开处理的现场记录。')}[c]||msg('内容を確認しながら使う項目です。','Use this while checking the details.','請邊確認內容邊使用。','请边确认内容边使用。');}

/* Illustration asset helpers. */
const RN_ILLUST_VERSION = 'v78m70';
...
function rnSvgImg(path, cls, alt){
  ...
  return `<span class="${esc(cls||'rn-svg')} rn-svg-missing">${msg('画像未設定','Image not set','圖片未設定','图片未设置')}: ${esc(category+'/'+name)}</span>`;
}
...
function mascotMarkup(name, alt=''){
  const defs = {
    word:{body:'#fffaf0',accent:'#f4b6c2',motif:'onigiri'},
    rice:{body:'#fff7d6',accent:'#f6c667',motif:'bowl'},
    fortune:{body:'#ffd7e8',accent:'#b794f4',motif:'star'},
    weather:{body:'#d7f2da',accent:'#74bde8',motif:'drop'},
    future:{body:'#ffe680',accent:'#8ccf7a',motif:'sprout'},
    country:{body:'#dff3ff',accent:'#f6a6a6',motif:'pin'},
    literature:{body:'#fffaf0',accent:'#8ecae6',motif:'book'},
    varieties:{body:'#f5c889',accent:'#9fd08b',motif:'lens'},
    water:{body:'#c9e7ff',accent:'#62b6cb',motif:'drop'},
    storage:{body:'#ffe08a',accent:'#caa66a',motif:'bag'},
    temp:{body:'#c8efd0',accent:'#f6c667',motif:'map'},
    ranking:{body:'#efc28b',accent:'#f6c667',motif:'cup'},
    glossary:{body:'#ffd7e8',accent:'#b794f4',motif:'book'},
    hero:{body:'#fff7d6',accent:'#8ccf7a',motif:'rice'}
  };
  const d=defs[name]||defs.rice;
  const motif={
    onigiri:`<path d="M49 25 25 69c-3 7 2 15 10 15h26c8 0 13-8 10-15L48 25Z" fill="#fff" stroke="#7b4f2a" stroke-width="4"/><rect x="39" y="63" width="18" height="18" rx="4" fill="#3f3a2f"/>`,
    bowl:`<path d="M25 52h46c-2 19-11 29-23 29S27 71 25 52Z" fill="#fff" stroke="#7b4f2a" stroke-width="4"/><ellipse cx="48" cy="52" rx="23" ry="9" fill="#fff" stroke="#7b4f2a" stroke-width="4"/><circle cx="37" cy="42" r="4" fill="#fff"/><circle cx="47" cy="38" r="4" fill="#fff"/><circle cx="58" cy="42" r="4" fill="#fff"/>`,
    star:`<path d="m40 22 4 10 10 4-10 4-4 10-4-10-10-4 10-4 4-10Z" fill="#ffd166" stroke="#7b4f2a" stroke-width="3"/>`,
    drop:`<path d="M70 24c9 11 14 19 14 28a14 14 0 0 1-28 0c0-9 5-17 14-28Z" fill="#bde9ff" stroke="#477b9a" stroke-width="3"/>`,
    sprout:`<path d="M49 35c1-12 9-17 21-18-2 12-9 18-21 18Z" fill="#94d27a" stroke="#548c45" stroke-width="3"/><path d="M49 36c-1-10-8-15-18-15 1 11 8 16 18 15Z" fill="#b8e994" stroke="#548c45" stroke-width="3"/>`,
    pin:`<path d="M70 23c8 0 14 6 14 14 0 11-14 26-14 26S56 48 56 37c0-8 6-14 14-14Z" fill="#ffb4a2" stroke="#7b4f2a" stroke-width="3"/><circle cx="70" cy="37" r="5" fill="#fff"/>`,
    book:`<path d="M25 31c11-5 19-4 23 2v38c-6-5-14-7-23-3V31Z" fill="#fff" stroke="#7b4f2a" stroke-width="3"/><path d="M49 33c6-6 14-7 23-2v37c-9-4-17-2-23 3V33Z" fill="#fff" stroke="#7b4f2a" stroke-width="3"/>`,
    lens:`<circle cx="64" cy="34" r="12" fill="#fff" stroke="#7b4f2a" stroke-width="4"/><path d="M73 43 84 54" stroke="#7b4f2a" stroke-width="5" stroke-linecap="round"/>`,
    bag:`<path d="M30 36h36l6 45H24l6-45Z" fill="#f5dfb8" stroke="#7b4f2a" stroke-width="4"/><path d="M35 36c2-9 24-9 26 0" fill="none" stroke="#7b4f2a" stroke-width="4"/><path d="M38 58h20" stroke="#7b4f2a" stroke-width="3" stroke-linecap="round"/>`,
    map:`<rect x="54" y="28" width="30" height="24" rx="5" fill="#fff" stroke="#7b4f2a" stroke-width="3"/><path d="M61 46 68 36l8 7" fill="none" stroke="#8ccf7a" stroke-width="3"/>`,
    cup:`<path d="M58 28h22v10c0 13-8 22-11 22s-11-9-11-22V28Z" fill="#ffd166" stroke="#7b4f2a" stroke-width="3"/><path d="M80 34h8c0 9-4 14-10 15" fill="none" stroke="#7b4f2a" stroke-width="3"/><path d="M65 60h8v9h8" stroke="#7b4f2a" stroke-width="3" fill="none"/>`,
    rice:`<ellipse cx="68" cy="35" rx="15" ry="24" fill="#fff" stroke="#7b4f2a" stroke-width="3" transform="rotate(20 68 35)"/>`
  }[d.motif]||'';
  return `<svg viewBox="0 0 96 96" role="img" aria-label="${esc(alt||name)}" xmlns="http://www.w3.org/2000/svg">
    <rect width="96" height="96" rx="24" fill="#fffaf4"/>
    <circle cx="78" cy="18" r="9" fill="${d.accent}" opacity=".22"/><circle cx="18" cy="77" r="12" fill="${d.accent}" opacity=".18"/>
    <g class="mascot-body"><path d="M21 51c0-18 12-31 27-31s27 13 27 31c0 19-12 31-27 31S21 70 21 51Z" fill="${d.body}" stroke="#7b4f2a" stroke-width="4" stroke-linecap="round"/>
    <circle cx="39" cy="49" r="3.2" fill="#4b2e1f"/><circle cx="57" cy="49" r="3.2" fill="#4b2e1f"/><path d="M44 60c3 4 7 4 10 0" fill="none" stroke="#4b2e1f" stroke-width="3" stroke-linecap="round"/><circle cx="32" cy="56" r="4" fill="#f7b7ad" opacity=".65"/><circle cx="64" cy="56" r="4" fill="#f7b7ad" opacity=".65"/></g>
    <g>${motif}</g>
  </svg>`;
}
function homeArt(name, cls='home-custom-art', alt=''){
  return `<span class="${esc(cls)} rn-mascot" aria-label="${esc(alt||name)}">${mascotMarkup(name, alt)}</span>`;
}
...
function storyCard(x,open=false){
  ...
  return `<div class="card story-card visual-card">${art}<h2>今日の世界ライス物語</h2><p class="small">${esc(field(x,'day_no')||'')}日目 / ${esc(storyCountry(x))} / ${esc(field(x,'region')||'')}</p><h3>${esc(storyTitle(x))}</h3><p><b>${esc(storySubtitle(x))}</b></p><p>${esc(storyBody(x,open?2000:520)).split('\\n').join('<br>')}</p><details ${open?'open':''}><summary>炊飯・食感の視点</summary><p><b>学び：</b>${esc(field(x,'learning_point')||'詳細で確認できます')}</p><p><b>食感：</b>${esc(field(x,'texture_note')||'詳細で確認できます')}</p><p><b>炊飯：</b>${esc(field(x,'cooking_note')||'詳細で確認できます')}</p><p>${chips([field(x,'texture_keywords'),field(x,'related_terms')])}</p><p class="small">本文のあとに、炊飯・食感の見方を添えています。</p></details><div class="priority-row">${btn('future','物語一覧へ')}${btn('varieties','品種を見る')}${btn('words','米辞典へ')}</div></div>`;
}


function worldRiceHeroImage(){
  return `<figure class="home-story-hero-image" aria-label="世界のライス物語">
    <picture>
      <source srcset="assets/illustrations/world_rice_story_hero.webp?v=78m70" type="image/webp">
      <img src="assets/illustrations/world_rice_story_hero.jpg?v=78m70" alt="世界の米文化と料理をめぐる、やさしいキャラクターたちのイラスト" loading="eager" decoding="async">
    </picture>
  </figure>`;
}
...
function varietyProfile(v){
  const name=varietyBaseName(v||{});
  return VARIETY_PUBLIC_PROFILES[name] || null;
}
...
function varietyFeatureLabel(v){
  ...
  const summary=cleanPublicText(field(v,'app_summary_ja_v1_6','safe_app_note_ja_v1_4','app_summary_ja_v1_7'));
...
function varietyReadableSummary(v){
  ...
  const summary=cleanPublicText(field(v,'app_summary_ja_v1_6','app_summary_ja_v1_7'));
  if(summary && !isBadSummaryText(summary)) return summary;
...
function conditionSummary(){
  return {title:t('waterRice'), body:msg('現在地や選んだ地域から、水質の公表ページを出します。値が登録済みなら硬度・pH・残留塩素を表示し、数値がない地域では公式リンクから確認できます。最後は、その水で炊く時の硬さ・粘り・香り・水加減の見方へつなげます。','Shows public water-quality pages for your current or selected area. If values are registered, hardness, pH, and residual chlorine are shown; if not, the app guides you to official links. The final view connects the water to firmness, stickiness, aroma, and water adjustment when cooking rice.','依所在地或選擇地區顯示公開水質頁面。有登錄數值時顯示硬度、pH與餘氯；沒有數值時引導到官方連結。最後會連到用這種水煮飯時的硬度、黏性、香氣與水量調整觀察。','按所在地或选择地区显示公开水质页面。有登记数值时显示硬度、pH与余氯；没有数值时引导到官方链接。最后会连接到用这种水煮饭时的硬度、黏性、香气与水量调整观察。')};
}
...
function dailyCardsHTML(variety,learn,lit,term,future){
  ...
  const wordHTML=word?`<button class="daily-mini daily-word" onclick="setGlossarySearch('${esc(word.title||word.meaning||'')}')">${homeArt('word','daily-illust','米言葉')}<span>${t('todayRiceWord')}</span><b>${esc(word.title)}</b><em>${esc(word.meaning)}<br>${esc(word.message)}</em></button>`:'';
  const fortuneHTML=ft?`<button class="daily-mini daily-fortune" onclick="setFortuneSearch('${esc(ft.process||ft.rice||ft.title)}')">${homeArt('fortune','daily-illust','米占い')}<span>${t('todayFortune')}</span><b>${esc(ft.title)}</b><em>${t('luckyNumber')}：${esc(ft.number)} / ${t('luckyRice')}：${esc(ft.rice)} / ${t('luckyProcess')}：${esc(ft.process)}<br>${esc(ft.one)}<br>${esc(ft.message)}</em></button>`:'';
  return `<section class="home-section"><div class="section-head"><h2>${t('dailyDiscovery')}</h2><button class="link-btn" onclick="switchView('words')">${t('toDictionary')}</button></div>
  <div class="daily-card-row">
    ${wordHTML}
    ${fortuneHTML}
    <button class="daily-mini daily-rice" onclick="setVarietySearch('${esc(vf.title)}')">${homeArt('rice','daily-illust',t('todayRice'))}<span>${t('todayRice')}</span><b>${esc(vf.title)}</b><em>${esc(vf.meta)} / ${esc(vf.body)}</em></button>
    <button class="daily-mini daily-condition" onclick="switchView('check')">${homeArt('weather','daily-illust','米コンディション')}<span>${t('todayCondition')}</span><b>${esc(cond.title)}</b><em>${esc(cond.body)}</em></button>
    <button class="daily-mini daily-future" onclick="setFutureDetail(${fidx<0?0:fidx})">${homeArt('future','daily-illust',t('riceFuture'))}<span>${t('riceFuture')}</span><b>${esc(fut.title)}</b><em>${esc(fut.body)}</em></button>
  </div></section>`;
}

function countryHistoryCard(){
  const title=t('locationHistory');
  const art = rnIllustration('country-history-card','card-visual',title);
  return `<div class="card visual-card">${art}<h2>${title}</h2><p><b>${msg('GPSで現在地の国を1日1回だけ確認し、その国のお米の歴史・文化・料理・品種へつなげる機能です。','Checks the current country about once a day and connects it to rice history, culture, dishes, and varieties.','每天約確認一次所在地國家，並連到該地的米食歷史、文化、料理與品種。','每天约确认一次所在地国家，并连接到当地的米食历史、文化、料理与品种。')}</b></p><p>${msg('表示単位は市区町村ではなく国・地域単位です。緯度経度を見せる機能ではなく、国判定を米文化の案内に変換します。','The display is by country or region, not by municipality. It does not show coordinates; it turns location into rice-culture guidance.','顯示單位為國家或地區，不是市區町村。此功能不顯示經緯度，而是把位置判定轉為米文化導覽。','显示单位为国家或地区，不是市区町村。此功能不显示经纬度，而是把位置判断转为米文化导览。')}</p><details open><summary>${msg('位置情報の扱い','How location is handled','位置資訊的處理','位置信息的处理')}</summary><p>${msg('常時取得は行いません。基本は1日1回、最後に取得した国をその日の参考国として使います。','Location is not collected continuously. In principle, the last detected country is used as the day’s reference.','不會持續取得位置。原則上以最後取得的國家作為當日參考地。','不会持续获取位置。原则上以最后获取的国家作为当天参考地。')}</p><p class="warn">${msg('位置情報は現在地の目安です。水質や保管状態は、公表情報や実際の確認結果と分けて見ます。','Location is only a guide. Water quality and storage conditions are checked separately with public data or actual inspection.','位置資訊只是參考。水質與保存狀態需與公開資料或實際確認分開判斷。','位置信息只是参考。水质与保存状态需与公开资料或实际确认分开判断。')}</p></details><p class="small">${msg('国・地域ごとに、米文化、代表料理、品種、ランキング、関連文献へ進めるようにします。情報が少ない地域では、まず読める物語や公表資料への案内を出します。','For each country or region, the app leads to rice culture, dishes, varieties, rankings, and references. If information is limited, it guides readers to available stories or public sources first.','依國家或地區連到米文化、代表料理、品種、排行與相關文獻。資訊較少時，先引導到可閱讀的故事或公開資料。','按国家或地区连接到米文化、代表料理、品种、排行与相关文献。信息较少时，先引导到可阅读的故事或公开资料。')}</p>${btn('future',t('worldStories'))}</div>`;
}

function exploreTile(view, key, title, text){
  return `<button class="explore-tile" onclick="switchView('${view}')">${homeArt(key,'explore-illust',title)}<b>${esc(title)}</b><span>${esc(text)}</span></button>`;
}
function riceWeatherConceptCard(){
  return card(t('todayCondition'),`${homeArt('weather','illust home-mini-illust',t('todayCondition'))}<p><b>${msg('天気予報ではなく、気象条件を米管理の注意へ変換します。','This is not a weather forecast; it converts weather conditions into rice-care cautions.','這不是天氣預報，而是把氣象條件轉換成米管理提醒。','这不是天气预报，而是把气象条件转换成米管理提醒。')}</b></p><p><b>${msg('保管注意','Storage note','保存提醒','保存提醒')}：</b>${msg('高湿度の日は米袋周辺、壁際、床、納米庫内壁、残米を確認します。','On humid days, check around rice bags, walls, floors, storage-bin inner walls, and remaining rice.','高濕度日要確認米袋周邊、牆邊、地面、納米庫內壁與殘米。','高湿度日要确认米袋周边、墙边、地面、纳米库内壁与残米。')}</p><p><b>${msg('結露注意','Condensation note','結露提醒','结露提醒')}：</b>${msg('外気と庫内・室内の温度差が大きい日は、水滴、濡れた付着米、カビ臭、虫、変色を先に見ます。','When the temperature gap is large, check for water drops, wet attached rice, mold odor, insects, and discoloration first.','外氣與庫內或室內溫差大時，先看水滴、濕黏米、霉味、蟲害與變色。','外气与库内或室内温差大时，先看水滴、湿黏米、霉味、虫害与变色。')}</p><p class="warn">${msg('GPSだけでは水質や庫内状態は分かりません。地域の参考値と実測値は分けて扱います。','GPS alone cannot determine water quality or storage-bin conditions. Treat local reference values and measured values separately.','只靠GPS無法判斷水質或庫內狀態。地區參考值與實測值需分開看。','只靠GPS无法判断水质或库内状态。地区参考值与实测值需分开看。')}</p>${btn('check',t('check'))}`);
}
...
async function load(){const res=await fetch('data/rice_navi_data.json?v=78m70');S.data=await res.json();document.querySelectorAll('.bottomnav button').forEach(b=>b.onclick=()=>switchView(b.dataset.view));$('#lang').onchange=e=>{S.lang=e.target.value;updateStaticI18n();render()};const rb=document.getElementById('refreshApp');if(rb)rb.onclick=refreshAppCache;updateStaticI18n();render();}
...
function renderHome(){
 ...
 $('#home').innerHTML=`<div class="home-app-shell">
   <section class="concept-story-hero">
     ${worldRiceHeroImage()}
     <div class="story-floating-card">
       ...
       <div class="story-copy">
         ...
         <p class="small">${esc(field(story,'day_no')||'')}${t('days')} / ${esc(storyCountry(story||{}))} / ${esc(field(story,'region')||'')}</p>
         <h2>${esc(storyTitle(story||{}))}</h2>
         <p>${esc(storyBody(story||{},230)).split('\n').join('<br>')}</p>
       </div>
       <button class="story-read" onclick="setStoryDetail(todayIndex(storyList().length,'world_rice_story'))">${t('readMore')}</button>
     </div>
   </section>
   ${dailyCardsHTML(variety,learn,lit,term,future)}
   <section class="home-section"><div class="section-head"><h2>${t('learnResearch')}</h2><button class="link-btn" onclick="switchView('learn')">${t('list')}</button></div>
     <div class="explore-grid">
       ${exploreTile('future','literature',t('worldStories'),t('worldStoriesDesc'))}
       ...
       ${exploreTile('learn','temp',t('riceStudy'),t('riceStudyDesc'))}
     </div>
   </section>
   <section class="home-record-card">
     <div><b>${t('yourRecord')}</b><p>${t('recordText')}</p></div>
     <div class="record-metrics"><span><b>${esc(counts.world_rice_stories||0)}</b>${t('stories')}</span><span><b>${esc(counts.rice_varieties||0)}</b>${t('varietiesShort')}</span><span><b>${esc(counts.literature_cards_v74||0)}</b>${t('litShort')}</span></div>
   </section>
   <details class="data-check"><summary>${t('dataCount')}</summary><p class="small">${t('dataCountNote')}</p><div class="countbar">${stat(counts.literature_cards_v74,msg('炊飯文献','References','炊飯文獻','煮饭文献'),t('literatureLibrary'))}${stat(counts.field_notes_v74,msg('現場記録','Field notes','現場紀錄','现场记录'),msg('文献とは別枠','Separate from references','與文獻分開','与文献分开'))}${stat(counts.learning_cards_v82,msg('学習カード','Learning cards','學習卡','学习卡'),t('riceStudy'))}${stat(counts.rice_varieties,t('varietyBook'),t('varietiesShort'))}${stat(counts.daily_rice_words,t('todayRiceWord'),msg('花言葉風100','100 flower-language-style entries','花語風100則','花语风100则'))}${stat(counts.rice_fortune_items,t('todayFortune'),msg('100件','100 items','100則','100条'))}${stat(counts.glossary_terms,t('glossary'),msg('用語','terms','用語','术语'))}${stat(counts.future_rice,t('riceFuture'),msg('未来テーマ','future themes','未來主題','未来主题'))}${stat(counts.world_rice_stories,t('worldStories'),'365話')}${stat(counts.water_rules,msg('水質ルール','Water rules','水質規則','水质规则'),t('waterRice'))}${stat(counts.mold_rules,msg('納米庫ルール','Storage rules','納米庫規則','纳米库规则'),t('storage'))}${stat(counts.ranking_definitions,t('ranking'),'世界の米')}${stat(counts.ranking_items_complete,t('ranking')+msg('明細',' rows','明細','明细'),msg('TOP10実データ','TOP 10 data rows','TOP10實際資料','TOP10实际数据'))}</div></details>
 </div>`;
}
...
function tempMapHTML(){
 ...
 return `<div class="section-title"><h2>${msg('温度×時間マップ','Temperature × time map','溫度×時間地圖','温度×时间地图')}</h2></div><div class="timeline">${events.map(e=>`<div class="step"><b>${esc(e[0])}</b><p>${esc(e[1])}</p><span>${esc(e[2])}</span><button class="btn secondary" onclick="switchView('${e[3]}')">${msg('関連を見る','See related items','查看相關','查看相关')}</button></div>`).join('')}</div>`;
}
function renderLearn(){
 ...
 const termTop=[
  ['浸漬',msg('浸漬','Soaking','浸泡','浸泡')],['吸水',msg('吸水','Absorption','吸水','吸水')],['水温',msg('水温','Water temperature','水溫','水温')],['糊化',msg('糊化','Gelatinization','糊化','糊化')],['蒸らし',msg('蒸らし','Resting','燜飯','焖饭')],['ほぐし',msg('ほぐし','Loosening','翻鬆','翻松')],['老化',msg('老化','Staling','老化','老化')],['硬さ',msg('硬さ','Firmness','硬度','硬度')],['粘り',msg('粘り','Stickiness','黏性','黏性')],['水質',msg('水質','Water quality','水質','水质')]
 ];
 $('#learn').innerHTML=`<div class="hero"><h1>${t('riceStudy')}</h1><p>${msg('初心者向けに、洗米・浸漬・水加減・加熱・蒸らし・ほぐし・保存を分かりやすく学ぶ場所です。文献ライブラリとは分け、根拠は必要な時だけ確認できる形にします。','A beginner-friendly place to learn washing, soaking, water ratio, heating, resting, loosening, and storage. It stays separate from the reference library, with sources available only when needed.','這裡以初學者容易理解的方式學習洗米、浸泡、水量、加熱、燜飯、翻鬆與保存。與文獻庫分開，只有需要時才確認根據。','这里以初学者容易理解的方式学习洗米、浸泡、水量、加热、焖饭、翻松与保存。与文献库分开，只有需要时才确认依据。')}</p></div><div class="data-note"><b>${msg('方針：','Policy: ','方針：','方针：')}</b>${msg('学ぶページは学ぶためのページです。あちこち飛ばさず、安全に言える基本だけを前面に出します。詳細な文献探索は文献ライブラリで行います。','The Learn page is for learning. It keeps the basics up front and leaves detailed source exploration to the reference library.','學習頁就是學習頁。先放可安全說明的基本內容，詳細文獻探索放在文獻庫。','学习页就是学习页。先放可安全说明的基本内容，详细文献探索放在文献库。')}</div>${tempMapHTML()}
 ...
 <div class="section-title"><h2>${ui('learnByProcess')}</h2></div><div class="priority-row">${termTop.map(([key,label])=>`<button onclick="S.filters.learn='${key}';renderLearn()">${label}</button>`).join('')}</div>
 <div class="toolbar"><input placeholder="${msg('教材検索','Search lessons','搜尋教材','搜索教材')}" value="${esc(q)}" oninput="setFilter('learn',this.value)"><select onchange="setFilter('level',this.value)"><option value="">${msg('全レベル','All levels','全部程度','全部程度')}</option>${levelKeys.map(k=>`<option value="${k}" ${k===level?'selected':''}>${levelLabelFromKey(k)}</option>`).join('')}</select></div>
 <p class="data-note">${msg('炊飯の勉強は初心者向けです。文献の細かい条件をそのまま並べず、根拠のある基本だけを読みやすく表示します。','The Learn page is for beginners. It does not list every technical condition from the references; it presents evidence-based basics in readable form.','炊飯學習頁面面向初學者。不直接並列文獻中的細部條件，而是把有根據的基本內容整理成容易閱讀的形式。','煮饭学习页面面向初学者。不直接并列文献中的细部条件，而是把有依据的基础内容整理成易读形式。')}</p><div class="countbar">${stat(filtered.length,msg('表示中の教材','Lessons shown','顯示中的教材','显示中的教材'))}${stat(data.length,msg('教材カード総数','Total lesson cards','教材卡總數','教材卡总数'))}${stat((S.data.literature.cards||[]).length,msg('参考文献','References','參考文獻','参考文献'),msg('詳細は文献へ','Details in references','詳情請到文獻','详情请到文献'))}</div>
 <div class="list">${filtered.map(x=>`<div class="item"><h3>${esc(lc(x,'title')||msg('教材名は詳細で確認','Check the lesson title in details','教材名稱請在詳情確認','教材名称请在详情确认'))}</h3><p>${chips([levelLabel(x),lc(x,'related_terms')])}</p><p>${esc(lc(x,'short')||lc(x,'easy')||msg('説明は詳細ページで確認します','Check the explanation on the detail page','說明請在詳情頁確認','说明请在详情页确认'))}</p><p><b>${msg('確認ポイント：','Check points: ','確認重點：','确认重点：')}</b>${esc(lc(x,'field_check_points')||msg('基本の確認ポイントは詳細で確認します','Check basic points in details','基本確認重點請在詳情確認','基础确认重点请在详情确认'))}</p><details><summary>${msg('条件・注意・出典','Conditions, notes, and sources','條件・注意・出處','条件・注意・出处')}</summary><p><b>${msg('条件・数値：','Conditions / numbers: ','條件・數值：','条件・数值：')}</b>${esc(lc(x,'numbers_conditions')||msg('必要な条件・数値は文献詳細または出典ページで確認します','Check necessary conditions and numbers in reference details or on the source page','必要條件與數值請在文獻詳情或出處頁面確認','必要条件与数值请在文献详情或出处页面确认'))}</p><p><b>${ui('note')}：</b>${esc(lc(x,'warning')||msg('注意点は詳細で確認します','Check notes in details','注意事項請在詳情確認','注意事项请在详情确认'))}</p><div class="source"><b>${msg('この教材の根拠','Sources for this lesson','此教材的根據','此教材的依据')}</b><br>${msg('元になった文献や根拠は文献ライブラリで確認できます。','The references and evidence behind this lesson can be checked in the reference library.','此教材依據的文獻與根據可在文獻庫確認。','此教材依据的文献与依据可在文献库确认。')}<br>${msg('参考資料：文献ライブラリの詳細で確認できます。','References: check the details in the reference library.','參考資料：請在文獻庫詳情確認。','参考资料：请在文献库详情确认。')}</div></details><button class="btn secondary" onclick="S.filters.litq='${esc((lc(x,'related_terms')||lc(x,'title')||'').split(/[;；,、]/)[0])}';switchView('literature')">${msg('説明のもとになった文献を確認する','Check the references behind this explanation','確認此說明依據的文獻','确认此说明依据的文献')}</button></div>`).join('')}</div>`;
}
...
function renderLiterature(){
 ...
 const detailHTML=detail?`<div class="card detail-card"><h2>${esc(literatureFieldText(detail,'title',msg('文献名','reference title','文獻名稱','文献名称'))||msg('文献名は詳細で確認','Check the reference title in details','文獻名稱請在詳情確認','文献名称请在详情确认'))}</h2><p>${chips([literatureCategoryLabel(detail.category),evidenceLabel(detail._class)])}</p><p class="small">${esc(statusInfo(detail._class))}</p><p class="data-note">${msg('このページは文献そのものを確認する場所です。学習ページでは省いた実験条件・数値・出典を、必要な時だけここで確認します。','This page is for checking the references themselves. Experimental conditions, numbers, and sources omitted from the Learn page are shown here only when needed.','此頁用來確認文獻本身。學習頁省略的實驗條件、數值與出處，只有需要時才在此確認。','此页用于确认文献本身。学习页省略的实验条件、数值与出处，仅在需要时于此确认。')}</p><p class="plain-box"><b>${msg('この文献で見ること：','What to check in this reference: ','此文獻要看的內容：','此文献要看的内容：')}</b>${msg('タイトル、要点、条件・数値、出典を確認し、学習カードの説明がどの資料に基づくかをたどります。','Check the title, key point, conditions, numbers, and sources, and trace which materials support the lesson explanation.','確認標題、要點、條件・數值與出處，追蹤學習卡說明依據哪些資料。','确认标题、要点、条件・数值与出处，追踪学习卡说明依据哪些资料。')}</p><p><b>${msg('文献要点：','Reference key point: ','文獻要點：','文献要点：')}</b>${esc(literatureFieldText(detail,'summary',msg('要点','key point','要點','要点'))&&!same(literatureFieldText(detail,'summary',msg('要点','key point','要點','要点')),literatureFieldText(detail,'title',msg('文献名','reference title','文獻名稱','文献名称')))?literatureFieldText(detail,'summary',msg('要点','key point','要點','要点')):msg('要点は文献詳細または出典ページで確認します','Check the key point in reference details or on the source page','要點請在文獻詳情或出處頁面確認','要点请在文献详情或出处页面确认'))}</p><p><b>${msg('条件・数値：','Conditions / numbers: ','條件・數值：','条件・数值：')}</b>${esc(literatureFieldText(detail,'condition',msg('条件・数値','conditions and numbers','條件・數值','条件・数值'))||msg('条件・数値は文献詳細または出典ページで確認します','Check conditions and numbers in reference details or on the source page','條件與數值請在文獻詳情或出處頁面確認','条件与数值请在文献详情或出处页面确认'))}</p><p><b>${msg('確認ポイント：','Check point: ','確認重點：','确认重点：')}</b>${esc(field(detail,'trace')?traceText(detail.trace):msg('この文献で何を確認できるかを、要点・条件・出典から確認します','Use the key point, conditions, and sources to see what this reference can confirm.','從要點、條件與出處確認此文獻可確認什麼。','从要点、条件与出处确认此文献可确认什么。'))}</p>${claimDetails(field(detail,'claim_ids'))}${sourceDetails(field(detail,'source_ids'))}<div class="priority-row"><button class="btn secondary" onclick="S.filters.learn='${esc((detail.category||detail.title||'').split(/[・/ ]/)[0])}';switchView('learn')">${msg('関連教材を探す','Find related lessons','尋找相關教材','寻找相关教材')}</button><button class="btn secondary" onclick="S.filters.wordq='${esc((detail.category||detail.title||'').split(/[・/ ]/)[0])}';switchView('words')">${msg('米辞典で見る','View in dictionary','在米辭典查看','在米词典查看')}</button><button class="btn secondary" onclick="S.filters.litDetail='';renderLiterature()">${msg('詳細を閉じる','Close details','關閉詳情','关闭详情')}</button></div></div>`:'';
 $('#literature').innerHTML=`<div class="hero"><h1>${t('literatureLibrary')}</h1><p>${msg('学習ページとは分けた、根拠データベースです。まず文献名を一覧で確認し、気になるタイトルを開いたときだけ条件・要点・根拠・出典を表示します。現場記録は文献と分けます。','This is an evidence database kept separate from the Learn page. First browse reference titles; open a title only when you want conditions, key points, evidence, and sources. Field notes are kept separate from references.','這是與學習頁分開的根據資料庫。先從文獻名稱列表確認，只有打開在意的標題時才顯示條件、要點、根據與出處。現場記錄與文獻分開。','这是与学习页分开的依据数据库。先从文献名称列表确认，只有打开关注的标题时才显示条件、要点、依据与出处。现场记录与文献分开。')}</p></div>
 <div class="countbar">${stat(classCounts.A,msg('条件まで見られる資料','References with conditions','可看條件的資料','可看条件的资料'),t('literature'))}${stat(classCounts.B,msg('要点がある資料','References with key points','有要點的資料','有要点的资料'),t('literature'))}${stat(classCounts.C,msg('出典・要点を詳細で確認する資料','References needing detail check','需在詳情確認出處・要點的資料','需在详情确认出处・要点的资料'),t('literature'))}${stat(notes.length,msg('現場記録','Field notes','現場記錄','现场记录'),msg('文献と分離','Separate from references','與文獻分開','与文献分开'))}</div>
 <div class="toolbar"><input placeholder="${msg('文献検索','Search references','搜尋文獻','搜索文献')}" value="${esc(q)}" oninput="setFilter('litq',this.value)"><select onchange="setFilter('litcat',this.value)"><option value="">${ui('allCategories')}</option>${categories.map(c=>`<option ${c===cat?'selected':''}>${esc(c)}</option>`).join('')}</select><select aria-label="${msg('根拠確認ステータス','Evidence status','根據確認狀態','依据确认状态')}" onchange="setFilter('evidence',this.value)"><option value="AB" ${ev==='AB'?'selected':''}>${msg('読みやすい資料中心','Readable references first','以易讀資料為主','以易读资料为主')}</option><option value="A" ${ev==='A'?'selected':''}>${msg('条件まで見られる資料','References with conditions','可看條件的資料','可看条件的资料')}</option><option value="B" ${ev==='B'?'selected':''}>${msg('要点がある資料','References with key points','有要點的資料','有要点的资料')}</option><option value="C" ${ev==='C'?'selected':''}>${msg('出典・要点を詳細で確認する資料','References needing detail check','需在詳情確認出處・要點的資料','需在详情确认出处・要点的资料')}</option><option value="ALL" ${ev==='ALL'?'selected':''}>${msg('全件','All','全部','全部')}</option></select></div>
 ...
 ${detailHTML}
 <div class="countbar">${stat(filtered.length,msg('表示中の文献タイトル','Reference titles shown','顯示中的文獻標題','显示中的文献标题'))}${stat(cards.length,msg('文献カード総数','Total reference cards','文獻卡總數','文献卡总数'))}${stat(notes.length,msg('現場記録','Field notes','現場記錄','现场记录'))}</div>
 ...
 <details class="field-note"><summary>${msg('現場記録を別枠で見る','View field notes separately','另行查看現場記錄','另行查看现场记录')}</summary><p class="small">${msg('現場記録は、文献ライブラリとは分けて読む補足情報です。根拠資料と混同しないよう別枠で表示します。','Field notes are supplementary information read separately from the reference library. They are shown in a separate section so they are not confused with evidence sources.','現場記錄是與文獻庫分開閱讀的補充資訊。為避免與根據資料混同，會另行顯示。','现场记录是与文献库分开阅读的补充信息。为避免与依据资料混同，会另行显示。')}</p><div class="list">${notes.map(n=>`<div class="item field-item"><h3>${esc(field(n,'title')||msg('メモ名は詳細で確認','Check note title in details','備忘名稱請在詳情確認','备注名称请在详情确认'))}</h3><p>${chips([n.category,traceText(n.decision),msg('文献とは分離','Separate from references','與文獻分開','与文献分开')])}</p><p>${esc(field(n,'summary')||'')}</p><p><b>${msg('扱い：','Treatment: ','處理方式：','处理方式：')}</b>${esc(field(n,'condition')||msg('詳細で確認できます','Check in details','可在詳情確認','可在详情确认'))}</p><p><b>${msg('次の確認：','Next check: ','下一步確認：','下一步确认：')}</b>${esc(field(n,'next_action')||msg('詳細で確認できます','Check in details','可在詳情確認','可在详情确认'))}</p></div>`).join('')}</div></details>`;
}
...
function renderVarieties(){
 ...
 $('#varieties').innerHTML=`<div class="hero"><h1>${t('varietyBook')}</h1><p>${msg(`${data.length}件を国別・用途別・食感別・アミロース・たんぱく質で確認します。`,`Check ${data.length} varieties by country, use, texture, amylose, and protein.`,`可依國家、用途、食感、直鏈澱粉與蛋白質確認${data.length}個品種。`,`可按国家、用途、口感、直链淀粉和蛋白质确认${data.length}个品种。`)}</p>${q?`<p class="data-note">${msg('検索中：','Searching: ','搜尋中：','搜索中：')}${esc(q)}。${msg('ホームの「今日のお米」から来た場合は、この条件で品種を表示します。','When opened from Today’s rice on the home screen, varieties are shown with this condition.','若從首頁「今日米種」進入，會以此條件顯示品種。','若从首页“今日米种”进入，会以此条件显示品种。')}</p>`:''}</div>
 ...
 <div class="list">${filtered.slice(0,250).map(v=>{const vs=varietyShort(v); const amy=varietyCompositionText(field(v,'amylose_range','amylose_value','amylose_class')); const pro=varietyCompositionText(field(v,'protein_range','protein_value','protein_class')); const feature=varietyFeatureDisplay(v); const use=varietyUseDisplay(v); const region=varietyRegionDisplay(v); const summary=varietySummaryDisplay(v); return `<div class="item variety-card"><h3>${esc(titleOfVariety(v))}</h3><p>${chips([region,use,feature])}</p><p class="variety-summary">${esc(summary)}</p><div class="plain-box variety-profile"><p><b>${msg('地域：','Region: ','地區：','地区：')}</b>${esc(region)}</p><p><b>${msg('特徴：','Traits: ','特徵：','特征：')}</b>${esc(feature)}</p><p><b>${msg('用途：','Use: ','用途：','用途：')}</b>${esc(use)}</p></div>${(amy||pro)?`<p><b>${msg('成分の手がかり：','Composition hints: ','成分線索：','成分线索：')}</b>${esc([amy?`${msg('アミロース','Amylose','直鏈澱粉','直链淀粉')} ${amy}`:'',pro?`${msg('たんぱく質','Protein','蛋白質','蛋白质')} ${pro}`:''].filter(Boolean).join(' / '))}</p>`:''}<details><summary>${msg('詳細・出典','Details and sources','詳情・出處','详情・出处')}</summary><p><b>${msg('品種名：','Variety name: ','品種名稱：','品种名称：')}</b>${esc(field(v,'variety_name','local_name','display_name_ja'))}</p><p><b>${msg('地域・市場：','Region / market: ','地區・市場：','地区・市场：')}</b>${esc(varietyDisplayText(field(v,'main_consumption_markets'))||region)}</p><p><b>${msg('出典の見方：','How to read sources: ','出處讀法：','出处读法：')}</b>${msg('この品種説明の根拠になった公的資料・研究資料・生産団体資料などを確認する欄です。数値や専門条件は、本文ではなくここで確認します。','This section points to public documents, research, producer information, and other sources behind the variety description. Check numbers and technical conditions here rather than in the short body text.','此欄用來確認支撐品種說明的公部門資料、研究資料、生產團體資料等。數值與專門條件不放在短文中，而在此確認。','此栏用于确认支撑品种说明的公共资料、研究资料、生产团体资料等。数值与专业条件不放在短文中，而在此确认。')}</p>${friendlySourceHTML(field(v,'source_urls'))}${field(v,'translation_status_variety_display')?`<p class="data-note">${msg('品種本文表示確認：m64再監査済みをm66でも維持','Variety text display: m64 re-audit retained in m66','品種本文顯示：m64再確認狀態於m66維持','品种正文显示：m64复核状态于m66维持')}</p>`:''}</details><p class="data-note">${esc(explainEvidenceLink('variety'))}</p><button class="btn secondary" onclick="S.filters.litq='${esc(field(v,'related_terms','rice_type','display_name_ja'))}';switchView('literature')">${msg('この品種の根拠・関連資料を見る','View sources and related references for this variety','查看此品種的根據與相關資料','查看此品种的依据与相关资料')}</button></div>`}).join('')}</div>`;
}
...
function renderCheck(){
 ...
 $('#check').innerHTML=`<div class="hero"><h1>${t('check')}</h1><p>${msg('GPS・天気・水質・保管情報を、そのまま出すのではなく、米の確認順に変換して表示します。','GPS, weather, water, and storage information are translated into the order of checks for rice.','GPS、天氣、水質與保管資訊不直接羅列，而是轉成米飯確認順序顯示。','GPS、天气、水质与保管信息不直接罗列，而是转成米饭确认顺序显示。')}</p></div>
 <div class="split">
 ${card(t('waterRice'),`${waterRegionPickerHTML(regions,selected)}${waterIntroHTML(region,w)}`,`<p class="small">${msg('水質ルール','Water rules','水質規則','水质规则')} ${(w.quality_rules||[]).length}${msg('件',' items','件','项')} / ${msg('水質根拠','Water references','水質根據','水质依据')} ${(w.claims||[]).length}${msg('件',' items','件','项')}</p>`)}
 ${card(msg('水質ルール一覧','Water-rule list','水質規則列表','水质规则列表'),`<p class="data-note">${msg('硬度・pHなどの数字を、炊飯でどう見ればよいかに置き換えて表示します。たとえばpHは7前後が中性、硬度は水に含まれるミネラルの多さです。','Hardness, pH and similar values are shown as rice-cooking hints. Around pH 7 is neutral, and hardness indicates the amount of minerals in water.','硬度、pH等數字會轉成炊飯時的觀察方式。pH約7為中性，硬度表示水中礦物質的多寡。','硬度、pH等数字会转成煮饭时的观察方式。pH约7为中性，硬度表示水中矿物质的多少。')}</p><div class="compact-list">${(w.quality_rules||[]).slice(0,13).map(r=>`<p>${esc(localizedWaterPhrase(r))}</p>`).join('')}</div><details><summary>${msg('水質出典','Water sources','水質出處','水质出处')}</summary>${(w.sources||[]).slice(0,10).map(s=>`<div class="source"><b>${msg('参考資料','Reference','參考資料','参考资料')}</b><br>${esc(field(s,'source_title_ja')||msg('資料名は詳細で確認します','Check source title in details','資料名稱可在詳情確認','资料名称可在详情确认'))}<br>${has(field(s,'source_url'))?`<a href="${esc(field(s,'source_url'))}" target="_blank" rel="noopener">${esc(field(s,'source_url'))}</a>`:msg('URLは出典ページで確認します','Check URL on the source page','URL請在出處頁面確認','URL请在出处页面确认')}</div>`).join('')}</details>`)}
 ${card(msg('保管環境チェック','Storage environment check','保管環境檢查','保管环境检查'),`<p><b>${msg('今日の保管注意：','Today’s storage note: ','今日保管注意：','今日保管注意：')}</b>${msg('湿度・温度差・雨天後の結露跡を確認します。','Check humidity, temperature difference, and condensation marks after rain.','確認濕度、溫差與雨後結露痕跡。','确认湿度、温差与雨后结露痕迹。')}</p><p>${msg('見る場所：米袋周辺、床、壁際、ホッパー、搬送部、納米庫内壁。','Check areas: around rice bags, floor, wall side, hopper, conveying sections, and inner walls of the rice-storage bin.','查看位置：米袋周邊、地面、牆邊、料斗、搬送部、納米庫內壁。','查看位置：米袋周边、地面、墙边、料斗、输送部、纳米库内壁。')}</p><p class="warn">${msg('濡れた付着米、カビ臭、虫、変色は炊飯調整ではなく保管・清掃・隔離判断です。','Wet rice residue, mold odor, insects, or discoloration are storage, cleaning, or isolation issues, not cooking adjustments.','濕的殘留米、霉味、蟲害、變色屬於保管、清掃或隔離判斷，不是炊飯調整。','湿的残留米、霉味、虫害、变色属于保管、清扫或隔离判断，不是煮饭调整。')}</p>`)}
 ${card(t('storage'),`<p><b>${msg('見る順番：','Check order: ','查看順序：','查看顺序：')}</b>${msg('結露 → 湿度 → 温度差 → 残米 → カビ臭 → 虫 → 変色 → 清掃記録。','Condensation → humidity → temperature difference → remaining rice → mold odor → insects → discoloration → cleaning record.','結露 → 濕度 → 溫差 → 殘米 → 霉味 → 蟲害 → 變色 → 清掃紀錄。','结露 → 湿度 → 温差 → 残米 → 霉味 → 虫害 → 变色 → 清扫记录。')}</p><div class="mini-grid">${storageStepLabels().map(x=>`<span>${esc(x)}</span>`).join('')}</div><details open><summary>${msg('危険サインと根拠','Danger signs and evidence','危險訊號與根據','危险信号与依据')}</summary>${coreClaims.slice(0,9).map(c=>`<p><b>${esc(c.theme||msg('確認','Check','確認','确认'))}</b>：${esc(c.claim_ja||msg('確認ポイントは詳細で確認します','Check points can be reviewed in details','確認重點可在詳情查看','确认重点可在详情查看'))}<br><span class="small">${msg('詳しい根拠は文献・出典情報で確認します。','Check detailed evidence in references and source information.','詳細根據請在文獻與出處資訊確認。','详细依据请在文献与出处信息确认。')}</span></p>`).join('')}</details><details><summary>${msg('点検場所','Inspection points','點檢位置','点检位置')}</summary>${checklist.slice(0,13).map(c=>`<p><b>${esc(c.timing||msg('点検','Inspection','點檢','点检'))}</b> ${esc(c.check_area||msg('場所は詳細で確認します','Check the place in details','位置請在詳情確認','位置请在详情确认'))}：${esc(c.check_item||msg('項目は詳細で確認します','Check the item in details','項目請在詳情確認','项目请在详情确认'))}<br><span class="small">${msg('方法：','Method: ','方法：','方法：')}${esc(c.method||msg('詳細で確認します','Check in details','請在詳情確認','请在详情确认'))} / ${msg('NG時：','If NG: ','NG時：','NG时：')}${esc(c.action_if_ng||msg('詳細で確認します','Check in details','請在詳情確認','请在详情确认'))}</span></p>`).join('')}</details>`,`<p class="small">${msg('判定ルール','Decision rules','判定規則','判定规则')} ${rules.length}${msg('件',' items','件','项')} / ${msg('点検','Inspection items','點檢','点检')} ${checklist.length}${msg('件',' items','件','项')} / ${msg('根拠','Evidence','根據','依据')} ${claims.length}${msg('件',' items','件','项')}</p>`)}
 ${card(msg('納米庫ルール一覧','Rice-storage rule list','納米庫規則列表','纳米库规则列表'),`<div class="compact-list">${rules.slice(0,21).map(r=>`<p><b>${esc(field(r,'rule_id','condition','trigger')||msg('ルール','Rule','規則','规则'))}</b>：${esc(field(r,'action_ja','rule_ja','message_ja','note_ja')||field(r,'condition')||msg('詳細で確認します','Check in details','請在詳情確認','请在详情确认'))}</p>`).join('')}</div><details><summary>${msg('納米庫出典','Rice-storage sources','納米庫出處','纳米库出处')}</summary>${sources.slice(0,10).map(s=>`<div class="source"><b>${msg('参考資料','Reference','參考資料','参考资料')}</b><br>${esc(s.title||msg('資料名は詳細で確認します','Check source title in details','資料名稱可在詳情確認','资料名称可在详情确认'))}<br>${has(s.url)?`<a href="${esc(s.url)}" target="_blank" rel="noopener">${esc(s.url)}</a>`:msg('URLは出典ページで確認します','Check URL on the source page','URL請在出處頁面確認','URL请在出处页面确认')}</div>`).join('')}</details>`)}
 ${card(msg('トラブル診断','Trouble guide','問題診斷','问题诊断'),`<p>${msg('症状から、関連する教材・文献・チェック項目へ進みます。','Start from a symptom and open related lessons, references, and check items.','從症狀進入相關教材、文獻與檢查項目。','从症状进入相关教材、文献与检查项目。')}</p><div class="quickgrid"><button onclick="S.filters.litq='べたつき';switchView('literature')">${troubleLabel('sticky')}</button><button onclick="S.filters.litq='硬さ';switchView('literature')">${troubleLabel('hard')}</button><button onclick="S.filters.learn='吸水';switchView('learn')">${troubleLabel('absorption')}</button><button onclick="S.filters.litq='老化';switchView('literature')">${troubleLabel('staling')}</button><button onclick="S.filters.litq='水質';switchView('literature')">${troubleLabel('water')}</button><button onclick="S.filters.litq='カビ';switchView('check')">${troubleLabel('mold')}</button><button onclick="S.filters.litq='糊化';switchView('literature')">${troubleLabel('core')}</button><button onclick="S.filters.litq='蒸らし';switchView('learn')">${troubleLabel('resting')}</button></div><p class="small">${msg('症状の説明が足りない部分は、関連する文献・教材・納米庫管理へ接続します。','When a symptom needs more explanation, the app connects it to references, lessons, or storage management.','症狀說明不足時，會連到相關文獻、教材或納米庫管理。','症状说明不足时，会连接到相关文献、教材或纳米库管理。')}</p>`)}
 </div>`;
}
...
function renderFuture(){
 ...
 const storyDetailIndex=Number(S.filters.storyDetail); const selectedStory=Number.isFinite(storyDetailIndex)?stories[storyDetailIndex]:null;
 const futureDetailIndex=Number(S.filters.futureDetail); const selectedFuture=Number.isFinite(futureDetailIndex)?data[futureDetailIndex]:null;
 const storyDetailHTML=selectedStory?`<div class="card story-detail-card visual-card">${rnStoryArt(storyVisualNameV41i(selectedStory),'card-visual story-visual','世界のライス物語')}<h2>${esc(storyTitle(selectedStory))}</h2><p>${chips([field(selectedStory,'day_no')+'日目',storyCountry(selectedStory),field(selectedStory,'region'),field(selectedStory,'theme')])}</p><p><b>${esc(storySubtitle(selectedStory))}</b></p><p>${esc(storyBody(selectedStory,2400)).split('\n').join('<br>')}</p><details open><summary>${msg('炊飯・食感の視点','Cooking and texture view','炊飯與口感視角','煮饭与口感视角')}</summary><p><b>${msg('学び','Learning point','學習重點','学习重点')}：</b>${esc(field(selectedStory,'learning_point')||publicFallback(msg('学び','learning point','學習重點','学习重点')))}</p><p><b>${msg('好み・文化','Preference and culture','喜好・文化','喜好・文化')}：</b>${esc(field(selectedStory,'preference_viewpoint')||publicFallback(msg('好み・文化','preference and culture','喜好・文化','喜好・文化')))}</p><p><b>${msg('料理場面','Cooking scene','料理場景','料理场景')}：</b>${esc(field(selectedStory,'scene_note')||publicFallback(msg('料理場面','cooking scene','料理場景','料理场景')))}</p><p><b>${msg('食感','Texture','口感','口感')}：</b>${esc(field(selectedStory,'texture_note')||publicFallback(msg('食感','texture','口感','口感')))}</p><p><b>${msg('炊飯','Rice cooking','炊飯','煮饭')}：</b>${esc(field(selectedStory,'cooking_note')||publicFallback(msg('炊飯','rice cooking','炊飯','煮饭')))}</p><p>${chips([field(selectedStory,'texture_keywords'),field(selectedStory,'related_terms'),field(selectedStory,'related_card_search')])}</p></details><button class="btn secondary" onclick="S.filters.storyDetail='';renderFuture()">${ui('listBack')}</button></div>`:'';
 const futureDetailHTML=selectedFuture?`<div class="card future-detail-card"><h2>${esc(futureTitle(selectedFuture))}</h2><p>${chips([selectedFuture.category])}</p><p><b>${esc(futureSubtitle(selectedFuture))}</b></p><p>${esc(futureBody(selectedFuture))}</p><details open><summary>${msg('関連・出典','Related items and sources','相關・出處','相关・出处')}</summary><p><b>${ui('related')}：</b>${msg('米品種、ランキング、水と炊飯、文献ライブラリへ接続します。','Connects to varieties, rankings, water and rice cooking, and the reference library.','可連到米品種、排行、水與炊飯、文獻庫。','可连接到米品种、排行、水与煮饭、文献库。')}</p><div class="source">${esc(field(selectedFuture,'source_note','source_name')||msg('詳細で確認できます','Check in details','請在詳情確認','请在详情确认'))}<br>${esc(field(selectedFuture,'source_url')||msg('URLは出典ページで確認します','Check URL on the source page','URL請在出處頁面確認','URL请在出处页面确认'))}</div></details><div class="priority-row"><button class="btn secondary" onclick="S.filters.varq='${esc((futureTitle(selectedFuture)||selectedFuture.category||'').split(/[・/ ]/)[0])}';switchView('varieties')">${msg('関連品種を見る','See related varieties','查看相關品種','查看相关品种')}</button><button class="btn secondary" onclick="S.filters.litq='${esc((futureTitle(selectedFuture)||selectedFuture.category||'').split(/[・/ ]/)[0])}';switchView('literature')">${msg('関連文献を見る','See related references','查看相關文獻','查看相关文献')}</button><button class="btn secondary" onclick="S.filters.futureDetail='';renderFuture()">${ui('listBack')}</button></div></div>`:'';
 $('#future').innerHTML=`<div class="hero"><h1>${ui('worldFutureTitle')}</h1><p>${msg('一覧ではタイトルと短い導入だけを見せます。本文は、読みたいタイトルを押した詳細ページで確認します。ホームで選んだテーマは、その詳細が先に開きます。','Lists show titles and short leads only. Open a title to read the body. Items selected from the home screen open their detail first.','列表只顯示標題與短導入。正文可在想閱讀的標題詳情中確認。從首頁選擇的主題會先開啟其詳情。','列表只显示标题与短导入。正文可在想阅读的标题详情中确认。从首页选择的主题会先打开其详情。')}</p></div>
 <div class="grid">${storyCard(story,false)}${countryHistoryCard()}${card(msg('GPS・天気情報の扱い','How GPS and weather are used','GPS・天氣資訊的使用方式','GPS・天气信息的使用方式'),`<p><b>${msg('GPSで現在地の国・地域を確認し、その地域に関係する米文化や公表情報へ進めます。','GPS is used to identify your current country or region and connect to related rice culture and public information.','GPS用來確認目前所在國家或地區，並連到相關米文化與公開資訊。','GPS用于确认当前所在国家或地区，并连接到相关米文化与公开信息。')}</b></p><p>${msg('水質は、登録済み参考値がある地域では数値を表示し、アプリ内に数値がない地域では公表ページへのリンクを出します。天気情報は、保管注意や炊飯で見るポイントに変換します。','For water quality, stored areas show reference values; areas without stored values show public-page links. Weather is translated into storage cautions and rice-cooking checks.','水質方面，有登錄參考值的地區會顯示數值；App內無數值的地區會顯示公開頁面連結。天氣資訊會轉成保管注意與炊飯觀察點。','水质方面，有登记参考值的地区会显示数值；App内无数值的地区会显示公开页面链接。天气信息会转成保管注意与煮饭观察点。')}</p><p class="small">${msg('常時取得ではなく、必要な時に現在地を確認する設計です。','Location is checked only when needed, not continuously.','設計上不是持續取得位置，而是在需要時確認。','设计上不是持续获取位置，而是在需要时确认。')}</p>`)}</div>
 ${storyDetailHTML}${futureDetailHTML}
 ...
 <div class="list title-list">${storyFiltered.map(x=>{const idx=stories.indexOf(x);return `<div class="item story-item visual-item compact-title-item">${rnStoryArt(storyVisualNameV41i(x),'item-visual','世界のライス物語')}<h3>${esc(storyTitle(x))}</h3><p>${chips([field(x,'day_no')+'日目',storyCountry(x),field(x,'region'),field(x,'theme')])}</p><p><b>${esc(storySubtitle(x))}</b></p><button class="btn secondary" onclick="setStoryDetail(${idx})">${ui('read')}</button></div>`}).join('')}</div>
 <div class="section-title"><h2>${ui('future50')}</h2></div><div class="toolbar"><input placeholder="${ui('futureSearch')}" value="${esc(q)}" oninput="setFilter('future',this.value)"><select onchange="setFilter('futureCat',this.value)"><option value="">${ui('allCategories')}</option>${cats.map(c=>`<option ${c===cat?'selected':''}>${esc(c)}</option>`).join('')}</select></div><div class="countbar">${stat(filtered.length,ui('showing'))}${stat(data.length,ui('futureTotal'))}${stat(cats.length,ui('category'))}</div>
 <div class="list title-list">${filtered.map(x=>{const idx=data.indexOf(x);return `<div class="item compact-title-item"><h3>${esc(futureTitle(x))}</h3><p>${chips([x.category])}</p><p><b>${esc(futureSubtitle(x))}</b></p><button class="btn secondary" onclick="setFutureDetail(${idx})">${ui('readDetail')}</button></div>`}).join('')}</div>`;
}
...
function renderWords(){
 ...
 $('#words').innerHTML=`<div class="hero"><h1>${ui('riceDictionaryWords')}</h1><p>${msg('用語集は1000語をひとつの辞典として扱うメイン機能です。よく使う語も同じ辞典内で見つけやすい順に表示します。今日の米言葉とは分けて使います。','The dictionary treats all 1,000 terms as one integrated feature. Frequently used terms are surfaced in the same dictionary, separate from the daily rice words.','米辭典把1000個用語作為同一個功能整合呈現。常用詞也在同一辭典中優先顯示，與今日米語分開使用。','米词典把1000个术语作为同一个功能整合呈现。常用词也在同一词典中优先显示，并与今日米语分开使用。')}</p></div>
