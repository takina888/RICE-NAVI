...
const RN_ILLUST_VERSION = 'v78m89';
...
function exploreTile(view, key, title, text){
  return `<button type="button" class="explore-tile" data-go="${esc(view)}" onclick="return goView('${esc(view)}')" aria-label="${esc(title)}">${homeArt(key,'explore-illust',title)}<b>${esc(title)}</b><span>${esc(text)}</span></button>`;
}
...
function normalizeView(v){
  const aliases={dictionary:'words',glossary:'words',terms:'words',story:'future',stories:'future',futureStory:'future',condition:'check',confirm:'check',encyclopedia:'varieties'};
  return aliases[v]||v;
}
function goView(v){switchView(normalizeView(v));return false;}
function switchView(v){v=normalizeView(v);const el=$('#'+v); if(!el){console.warn('missing view',v); v='home';} S.view=v;document.querySelectorAll('.view').forEach(x=>x.classList.remove('active'));const target=$('#'+v);if(target)target.classList.add('active');document.querySelectorAll('.bottomnav button').forEach(b=>b.classList.toggle('active',b.dataset.view===v));render();window.scrollTo({top:0,behavior:'smooth'});}
...
async function load(){const res=await fetch('data/rice_navi_data.json?v=78m89');S.data=await res.json();document.querySelectorAll('.bottomnav button').forEach(b=>b.onclick=()=>switchView(b.dataset.view));document.addEventListener('click',e=>{const n=e.target.closest('[data-go]');if(!n)return;const inner=e.target.closest('button,a,input,select,textarea,summary');if(inner&&inner!==n)return;const v=n.getAttribute('data-go');if(v){e.preventDefault();switchView(normalizeView(v));}});document.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!==' ')return;const n=e.target.closest('[data-go]');if(!n)return;const inner=e.target.closest('button,a,input,select,textarea,summary');if(inner&&inner!==n)return;e.preventDefault();switchView(normalizeView(n.getAttribute('data-go')));});$('#lang').onchange=e=>{S.lang=e.target.value;updateStaticI18n();render()};const rb=document.getElementById('refreshApp');if(rb)rb.onclick=refreshAppCache;updateStaticI18n();render();}
...
function renderLearn(){
 ...
 $('#learn').innerHTML=`<div class="hero">${homeArt('learning_lesson','card-visual','炊飯の勉強')}<h1>${t('riceStudy')}</h1><p>${msg('洗米・浸漬・水加減・加熱・蒸らし・ほぐし・保存を、初心者にも分かりやすく確認できるページです。','A beginner-friendly place to review washing, soaking, water ratio, heating, resting, loosening, and storage.','這裡可用初學者也容易理解的方式，確認洗米、浸泡、水量、加熱、燜飯、翻鬆與保存。','这里可用初学者也容易理解的方式，确认洗米、浸泡、水量、加热、焖饭、翻松与保存。')}</p></div>${tempMapHTML()}
 ...
 <p class="data-note">${msg('炊飯の勉強は、基本を短く確認するためのページです。細かい条件は必要な時に文献ライブラリで確認できます。','The Learn page is for beginners. It does not list every technical condition from the references; it presents evidence-based basics in readable form.','炊飯學習頁面面向初學者。不直接並列文獻中的細部條件，而是把有根據的基本內容整理成容易閱讀的形式。','煮饭学习页面面向初学者。不直接并列文献中的细部条件，而是把有依据的基础内容整理成易读形式。')}</p><div class="countbar">${stat(filtered.length,msg('表示中の学び','Lessons shown','顯示中的學習','显示中的学习'))}${stat(data.length,msg('学び項目総数','Total learning items','學習項目總數','学习项目总数'))}${stat((S.data.literature.cards||[]).length,msg('参考文献','References','參考文獻','参考文献'),msg('詳細は文献へ','Details in references','詳情請到文獻','详情请到文献'))}</div>
 <div class="list">${filtered.map(x=>`<div class="item"><h3>${esc(lc(x,'title')||msg('項目名は詳細で確認','Check the lesson title in details','項目名稱請在詳情確認','项目名称请在详情确认'))}</h3><p>${chips([levelLabel(x),lc(x,'related_terms')])}</p><p>${esc(lc(x,'short')||lc(x,'easy')||msg('説明は詳細ページで確認します','Check the explanation on the detail page','說明請在詳情頁確認','说明请在详情页确认'))}</p><p><b>${msg('確認ポイント：','Check points: ','確認重點：','确认重点：')}</b>${esc(lc(x,'field_check_points')||msg('基本の確認ポイントは詳細で確認します','Check basic points in details','基本確認重點請在詳情確認','基础确认重点请在详情确认'))}</p><details><summary>${msg('条件・注意・出典','Conditions, notes, and sources','條件・注意・出處','条件・注意・出处')}</summary><p><b>${msg('条件・数値：','Conditions / numbers: ','條件・數值：','条件・数值：')}</b>${esc(lc(x,'numbers_conditions')||msg('必要な条件・数値は文献詳細または出典ページで確認します','Check necessary conditions and numbers in reference details or on the source page','必要條件與數值請在文獻詳情或出處頁面確認','必要条件与数值请在文献详情或出处页面确认'))}</p><p><b>${ui('note')}：</b>${esc(lc(x,'warning')||msg('注意点は詳細で確認します','Check notes in details','注意事項請在詳情確認','注意事项请在详情确认'))}</p><div class="source"><b>${msg('この内容の出典','Sources for this item','此內容的根據','此内容的依据')}</b><br>${msg('元になった文献や出典は文献ライブラリで確認できます。','The references and evidence behind this item can be checked in the reference library.','此內容依據的文獻與根據可在文獻庫確認。','此内容依据的文献与依据可在文献库确认。')}<br>${msg('参考資料：文献ライブラリの詳細で確認できます。','References: check the details in the reference library.','參考資料：請在文獻庫詳情確認。','参考资料：请在文献库详情确认。')}</div></details><button class="btn secondary" onclick="S.filters.litq='${esc((lc(x,'related_terms')||lc(x,'title')||'').split(/[;；,、]/)[0])}';switchView('literature')">${msg('説明のもとになった文献を確認する','Check the references behind this explanation','確認此說明依據的文獻','确认此说明依据的文献')}</button></div>`).join('')}</div>`;
}
...
function renderFuture(){
 ...
 <div class="list title-list">${filtered.map(x=>{const idx=data.indexOf(x);return `<div class="item compact-title-item visual-item">${homeArt('future_farming','item-visual','お米の未来')}<h3>${esc(futureTitle(x))}</h3><p>${chips([x.category])}</p><p><b>${esc(futureSubtitle(x))}</b></p><button class="btn secondary" onclick="setFutureDetail(${idx})">${ui('readDetail')}</button></div>`}).join('')}</div>`;
}
