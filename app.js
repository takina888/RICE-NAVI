...
(function(){
  function cleanView(v){
    ...
    var aliases={dictionary:'varieties',glossary:'words',terms:'words',word:'words',story:'future',stories:'future',futureStory:'future',worldStories:'future',condition:'check',confirm:'check',water:'check',storage:'check',encyclopedia:'varieties',variety:'varieties',riceVarieties:'varieties',ranking:'rankings'};
    v=aliases[v]||v;
  ...
  document.addEventListener('click',function(ev){
    ...
    // CLICKFIX01: do not intercept elements that already have inline onclick logic.
    // Daily cards use onclick to set search/detail filters before navigation.
    // The old capture-phase bridge stopped those handlers, so buttons opened the page
    // but lost the intended search/detail state. Let inline handlers run normally.
  ...
  document.addEventListener('keydown',function(ev){
    ...
    var el=ev.target && ev.target.closest && ev.target.closest('[data-action="back"],button[data-view],[data-go],a[href^="#"]');
    if(!el) return;
    // CLICKFIX01: preserve inline onclick behavior for keyboard activation too.
    if(el.getAttribute && el.getAttribute('onclick')) return;
    performNav(el,ev);
  },true);
})();
...
function updateStaticI18n(){
  ...
  const tag=document.querySelector('.brand .brand-name'); if(tag) tag.textContent='RICE NAVI';
...
function publicFallback(label){ return ''; }
function sourceFallback(){ return ''; }
function tx(obj,key){
  if(!obj) return '';
  const L=S.lang;
...
function traceText(x){
  return String(x||'')
    .replace(/出典\+出典確認できる資料/g,'条件と出典をたどれる資料')
    .replace(/出典\+出典確認済み/g,'条件と出典をたどれる資料')
...
function publicLabel(x){
  ...
  if(!s) return '';
  const map={
    app_heuristic:'の目安', public_or_manual:'公表値または入力値', template:'地域データ', active:'表示',
...
function waterOfficialLinks(region){
  ...
  if(country.includes('台湾')||area.includes('新北')||area.includes('桃園')||area.includes('台北')){
    add('台湾自來水公司：水質即時資訊','https://www.water.gov.tw/wq/','地図から地域を選び、残留塩素・濁度などの最新情報を。');
    add('台湾自來水公司：平均水質','https://www.water.gov.tw/ch/WaterQuality?nodeId=4631','浄水場・地域ごとの平均水質を。硬度、pH、残留塩素などを見るページです。');
    add('環境部 飲用水全球資訊網：自來水公司水質資訊','https://dwsiot.moenv.gov.tw/watercompany_taiwan_list','市区・浄水場ごとの水質情報ページへ進む一覧です。');
    if(area.includes('台北')||area.includes('新北')) add('臺北自來水事業處：水質資訊','https://www.water.gov.taipei/Content_List.aspx?n=77AB4A57F76B9882','台北・一部周辺地域の採水検査、管網、浄水場情報を。');
    if(area.includes('桃園')||area.includes('中壢')) add('台湾自來水公司：平鎮浄水場の平均水質','https://www.water.gov.tw/ch/WaterQuality/Detail/1992?nodeId=4631','桃園・中壢方面で水質情報を確認できる浄水場ページです。実際の給水区域は公式ページで。');
  } else if(country.includes('日本')||area.includes('名古屋')||area.includes('大阪')){
    if(area.includes('名古屋')) add('名古屋市上下水道局：水質検査結果','https://www.water.city.nagoya.jp/category/suidousuisitsu/144524.html','名古屋市内の毎日検査・毎月検査・水質管理情報を。');
    ...
    if(area.includes('大阪')) add('大阪市水道局：水道水質検査結果のお知らせ','https://www.city.osaka.lg.jp/suido/page/0000014772.html','大阪市の給水栓・浄水場出口などの検査結果を。');
    if(area.includes('大阪')) add('大阪市水道局：水道水質基準について','https://www.city.osaka.lg.jp/suido/page/0000014774.html','水質基準項目の見方を。');
...
function waterRegionPickerHTML(regions, selected){
  return `<div class="toolbar"><label>${msg('見る地域','Area to check','確認地區','确认地区')} <select onchange="S.filters.waterRegion=this.value;renderCheck()">${regions.map((r,i)=>`<option value="${i}" ${i==selected?'selected':''}>${esc(localizedField(r,'area_name')||field(r,'region_id')||('地域'+(i+1)))}</option>`).join('')}</select></label><button class="btn secondary" onclick="getWaterLocationHint()">${msg('現在地を取得','Use current location','取得目前位置','获取当前位置')}</button></div><div id="geo-status" class="data-note">${msg('地域を選ぶと、水質情報と炊飯の見方がわかります。','After getting your location, choose the closest area and check public water information plus rice-cooking points.','取得位置後，選擇可能對應的地區，確認公開水質頁面與炊飯觀察重點。','获取位置后，选择可能对应的地区，确认公开水质页面与煮饭观察重点。')}</div>`;
}
function getWaterLocationHint(){
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    box.innerHTML=msg('地域を選ぶと、水質情報と炊飯の見方がわかります。','Location checked. Choose the closest area to show water-information links and rice-cooking points.','已確認目前位置。選擇可能對應的地區後，會顯示水質資訊確認處與炊飯重點。','已确认当前位置。选择可能对应的地区后，会显示水质信息确认处和煮饭重点。');
...
function explainEvidenceLink(kind='文献'){
  if(kind==='learn') return msg('この勉強項目は、文献や公的資料の要点を読みやすく整理したものです。詳しい条件や出典は文献ライブラリで。','This lesson organizes key points from references and public materials in an easy-to-read way. Detailed conditions and sources can be checked in the reference library.','此教材把文獻與公開資料的要點整理成容易閱讀的內容。詳細條件與出處可在文獻庫確認。','此教材把文献与公开资料的要点整理成易读内容。详细条件与出处可在文献库确认。');
  if(kind==='variety') return msg('関連する資料を探します。','Search references and materials related to this variety. Variety descriptions and reference details are kept on separate pages.','查找與此品種相關的文獻與資料。品種說明與文獻詳情會分開確認。','查找与此品种相关的文献与资料。品种说明与文献详情会分开确认。');
  return msg('詳しい出典や条件は、文献ライブラリの。','Detailed evidence and conditions can be checked in the reference library.','詳細根據與條件可在文獻庫詳情確認。','详细依据与条件可在文献库详情确认。');
}
...
function rangeText(min,max,unit){
  ...
  if(a) return `${a}${u}以上`;
  if(b) return `${b}${u}以下`;
  return msg('資料を見る','Check the range in the source material','範圍請在資料中確認','范围请在资料中确认');
}
...
function waterIntroHTML(region,w){
  ...
  return `<p><b>${msg('地域の水質情報を見ながら、その水で炊く時の硬さ・粘り・香り・水加減の見方を。','Use local water information to check cooking hints such as firmness, stickiness, aroma, and water adjustment.','參考地區水質資訊，確認炊飯時的硬度、黏性、香氣與水量調整。','参考地区水质信息，确认煮饭时的硬度、黏性、香气与水量调整。')}</b></p>
  <p>${msg('最初に地域を。に硬度・pH・残留塩素などの値が入っていれば地域の参考値として表示し、に数値がない場合は、公式・公表ページへの確認リンクを出します。','First check the area. If hardness, pH, residual chlorine, or similar values are stored in the app, they are shown as local reference values. If not, links to official or public pages are shown.','首先確認地區。若App內已有硬度、pH、餘氯等數值，會以地區參考值顯示；若沒有數值，會顯示官方或公開頁面的確認連結。','首先确认地区。若App内已有硬度、pH、余氯等数值，会以地区参考值显示；若没有数值，会显示官方或公开页面的确认链接。')}</p>
  ...
  <div class="plain-box"><b>${msg('','Data status for this area: ','此地區的登錄狀態：','此地区的数据状态：')}</b>${hasRegionValue?msg('','Some water-quality reference values are stored in the app.','App內有部分水質參考值。','App内有部分水质参考值。'):msg('この地域の数値はにはありません。下の公表情報リンクで硬度・pH・残留塩素などを。','Values for this area are not stored in the app. Use the public-information links below to check hardness, pH, residual chlorine, and similar items.','此地區的數值尚未收錄在App內。請使用下方公開資訊連結確認硬度、pH、餘氯等項目。','此地区的数值尚未收录在App内。请使用下方公开信息链接确认硬度、pH、余氯等项目。')}</div>
...
function visible(...vals){for(const v of vals){if(has(v))return v;}return '';}
function sourceBadge(label,count){return `<span class="badge">${esc(label)}：${esc(count??0)}${msg('件',' items','件','项')}</span>`;}

function sourceMap(){const m={};(S.data?.literature?.sources||[]).forEach(x=>{const id=field(x,'出典','id','SourceID'); if(id)m[id]=x;});return m;}
function claimMap(){const m={};(S.data?.literature?.claims||[]).forEach(x=>{const id=field(x,'出典','id','ClaimID'); if(id)m[id]=x;});return m;}
function splitIds(v){return String(v||'').split(/[;,;；、\s]+/).map(norm).filter(has);}
function sourceDetails(ids){const sm=sourceMap(); const arr=splitIds(ids).map(id=>sm[id]).filter(Boolean); if(!arr.length)return `<div class="source">${msg('参考資料','Reference: check details','參考資料：請在詳情確認','参考资料：请在详情确认')}</div>`; return arr.slice(0,4).map(x=>`<div class="source"><b>${msg('参考資料','Reference','參考資料','参考资料')}</b><br>${esc(sourceTitleText(x)||field(x,'source_title_ja','title_ja','title','source_name')||msg('資料名','Check source title in details','資料名稱可在詳情確認','资料名称可在详情确认'))}<br>${sourceTopicText(x)?`<span class="small">${esc(sourceTopicText(x))}</span><br>`:''}${esc(field(x,'source_url','url')||msg('URL','Check URL on the source page','URL請在出處頁面確認','URL请在出处页面确认'))}</div>`).join('');}
...
function claimDetails(ids){const cm=claimMap(); const arr=splitIds(ids).map(id=>cm[id]).filter(Boolean); if(!arr.length)return `<p><b>${msg('要点：','What this reference helps confirm: ','此文獻可確認的內容：','此文献可确认的内容：')}</b>${msg('詳しく見る','Check details','請在詳情確認','请在详情确认')}</p>`; return arr.slice(0,4).map(x=>`<div class="claim-box"><p><b>${msg('要点：','What this reference helps confirm: ','此文獻可確認的內容：','此文献可确认的内容：')}</b>${esc(claimText(x)||msg('要点は詳しく見る','Check key points in details','要點請在詳情確認','要点请在详情确认'))}</p>${conditionText(x)?`<p class="small"><b>${msg('条件・数値：','Conditions / numbers: ','條件・數值：','条件・数值：')}</b>${esc(conditionText(x))}</p>`:''}</div>`).join('');}
...
function classifyEvidence(x){
  ...
  const src=has(x.出典s), cid=has(x.出典s), sum=has(x.summary)&&!same(x.summary,x.title), cond=has(x.condition);
...
function appImage(name, cls, alt){ return ''; }
function rnSvgImg(path, cls, alt){ return ''; }
function rnIllustration(name, cls, alt){ return ''; }
function rnStoryArt(name, cls, alt){ return ''; }
function rnCountryArt(name, cls, alt){ return ''; }
function rnRiceTypeArt(name, cls, alt){ return ''; }
...
function homePictogram(name, cls='home-custom-art', alt=''){
  ...
  const labels = {
    glossary:'用語集', words:'用語集', varieties:'米品種図鑑', literature:'文献', story:'物語', hero:'物語', country:'現在地', location_map:'現在地', ranking:'ランキング', water:'水と炊飯', storage:'納米庫管理', storage_check:'納米庫管理', learn:'学ぶ', learning_lesson:'学ぶ', temp:'学ぶ', word:'米言葉', fortune:'米占い', rice:'今日のお米', weather:'米コンディション', future:'お米の未来', local_food:'地域の米文化'
  };
...
function homeArt(name, cls='home-custom-art', alt=''){ return ''; }
function literatureScore(x){let s=0;const c=classifyEvidence(x); if(c==='A')s+=10;if(c==='B')s+=6;if(has(x.condition))s+=4;if(has(x.summary)&&!same(x.summary,x.title))s+=3;if(has(x.出典s))s+=2;if(has(x.出典s))s+=2;return s;}
...
function countryOfVariety(v){return field(v,'country_ja','origin_country_ja','main_production_regions','country')||msg('国・地域','Check country / region details','可在詳情確認國家・地區','可在详情确认国家・地区');}
...
function termNote(x){const note=tx(x,'note');if(!note||/工程カード用の統一訳語|Preferred term|工程卡統一用語|工序卡统一用语/.test(note))return msg('炊飯工程や米の状態を見るときに使う基本用語です。関連する教材・文献と合わせて。','A basic term used when checking rice-cooking steps or rice condition. Check it with related lessons and references.','這是確認炊飯工序或米飯狀態時使用的基本用語。請搭配相關教材與文獻確認。','这是确认煮饭工序或米饭状态时使用的基本术语。请搭配相关教材与文献确认。');return note;}
...
function storyCountry(x){return field(x,'country_area')||msg('国・地域','Check country / region details','可在詳情確認國家・地區','可在详情确认国家・地区');}
...
function storyBody(x,limit=520){
  const b=String(field(x,'body')||msg('本文は詳細ページで','Check the body on the detail page','本文請在詳情頁確認','正文请在详情页确认'));
  if(b.length<=limit) return b;
  return b.slice(0,limit)+'…';
}
function storyCard(x,open=false){
  if(!x) return card('今日の世界ライス物語',`${rnIllustration('daily-story-card','card-visual','世界のライス物語')}<p><b>物語詳しく見る</b></p><p>世界の米文化・料理・食感・炊き方を1日1話で見られます。</p>`);
  const art = rnStoryArt(storyVisualNameV41i(x),'card-visual story-visual','世界のライス物語');
  return `<div class="card story-card visual-card">${art}<h2>今日の世界ライス物語</h2><p class="small">${esc(storyDayDateChip(x))} / ${esc(storyCountry(x))} / ${esc(field(x,'region')||'')}</p><h3>${esc(storyTitle(x))}</h3><p><b>${esc(storySubtitle(x))}</b></p><p>${esc(storyBody(x,open?2000:520)).split('\\n').join('<br>')}</p><details ${open?'open':''}><summary>炊飯・食感の視点</summary><p><b>学び：</b>${esc(field(x,'learning_point')||'')}</p><p><b>食感：</b>${esc(field(x,'texture_note')||'')}</p><p><b>炊飯：</b>${esc(field(x,'cooking_note')||'')}</p><p>${chips([field(x,'texture_keywords'),field(x,'related_terms')])}</p><p class="small">本文のあとに、炊飯・食感の見方を添えています。</p></details><div class="priority-row">${btn('future','物語一覧へ')}${btn('varieties','品種を見る')}${btn('words','辞典へ')}</div></div>`;
}


function worldRiceHeroImage(){ return ''; }

function storyLeadCard(x){
  const art = worldRiceHeroImage();
  if(!x) return `<div class="card story-card visual-card home-lead-story">${art}<div class="lead-kicker">いちばん読めるメインコンテンツ</div><h2>世界のライス物語</h2><p><b>物語詳しく見る</b></p><p>世界の米文化・料理・食感・炊き方を、ホームの先頭で読めます。</p>${btn('future','物語一覧へ')}</div>`;
...
const INTERNAL_WORDS=/候補||整理し|確認できる資料|未確認|関係は[「\"]?高|関係は「?高|データ上|source_type|status|priority|claim|field note|ref_id|||保留|未作成|表示しない|アプリ非表示|出典s|release_status|component_status|app_card_status/i;
...
function publicValue(x, fallback=''){
...
function friendlySourceHTML(urls){
  ...
  if(!list.length) return `<div class="source">${msg('参考資料','Reference','參考資料','参考资料')}：${msg('資料欄で','Check the source field','請在資料欄確認','请在资料栏确认')}</div>`;
...
function countryOfVariety(v){return field(v,'country_ja','origin_country_ja','country','country_en')||'地域は';}
...
function varietyUseLabel(v){
  ...
  if(!s) return msg('用途は','Uses can be checked in details.','用途可在詳情確認。','用途可在详情确认。');
...
function varietyFeatureLabel(v){
  ...
  return [rice,grain].filter(has).join('、') || '特徴は、用途・地域・出典情報と合わせて。';
}
function varietyReadableSummary(v){
  if(!v) return '品種図鑑で用途や特徴を。';
  ...
  const use=varietyUseLabel(v);
  const parts=[];
  if(region && region!=='地域') parts.push(`${name}は、${region}で知られる米品種です。`);
  else parts.push(`${name}は、米品種図鑑にされている品種です。`);
  if(feature && feature!=='特徴は、用途・地域・出典情報と合わせて。') parts.push(feature);
  if(use && use!==msg('用途は','Uses can be checked in details.','用途可在詳情確認。','用途可在详情确认。')) parts.push(`主な用途は${use}です。`);
  return parts.join(' ');
}
...
function varietyTermMapText(x){
  ...
  if(!s) return '';
  const maps={
    en:[['白ごはん','plain white rice'],['白飯','white rice'],['主食','staple rice'],['家庭用','home use'],['贈答用','gift use'],['業務用','foodservice use'],['加工用','processing use'],['香り米','aromatic rice'],['もち米','glutinous rice'],['短粒','short grain'],['中粒','medium grain'],['長粒','long grain'],['低アミロース','low amylose'],['高アミロース','high amylose'],['たんぱく質','protein'],['粘り','stickiness'],['甘み','sweetness'],['つや','gloss'],['食感','texture'],['硬さ','firmness'],['冷凍米飯','frozen cooked rice'],['加工米飯','processed cooked rice'],['','Check in details'],['詳細で確認','Check in details']],
    zh_tw:[['白ごはん','白飯'],['白飯','白飯'],['主食','主食'],['家庭用','家庭用'],['贈答用','禮品用'],['業務用','業務用'],['加工用','加工用'],['香り米','香米'],['もち米','糯米'],['短粒','短粒'],['中粒','中粒'],['長粒','長粒'],['低アミロース','低直鏈澱粉'],['高アミロース','高直鏈澱粉'],['たんぱく質','蛋白質'],['粘り','黏性'],['甘み','甜味'],['つや','光澤'],['食感','口感'],['硬さ','硬度'],['冷凍米飯','冷凍米飯'],['加工米飯','加工米飯'],['','可在詳情確認'],['詳細で確認','可在詳情確認']],
    zh_cn:[['白ごはん','白米饭'],['白飯','白米饭'],['主食','主食'],['家庭用','家庭用'],['贈答用','礼品用'],['業務用','商用'],['加工用','加工用'],['香り米','香米'],['もち米','糯米'],['短粒','短粒'],['中粒','中粒'],['長粒','长粒'],['低アミロース','低直链淀粉'],['高アミロース','高直链淀粉'],['たんぱく質','蛋白质'],['粘り','黏性'],['甘み','甜味'],['つや','光泽'],['食感','口感'],['硬さ','硬度'],['冷凍米飯','冷冻米饭'],['加工米飯','加工米饭'],['','可在详情确认'],['詳細で確認','可在详情确认']]
  };
  if(S.lang==='ja') return s;
...
function varietyRegionDisplay(v){
  ...
  if(!parts.length) return msg('地域は','Check region in details','地區可在詳情確認','地区可在详情确认');
  return parts.map((part,i)=>i===0?localizeCountryName(part):varietyDisplayText(part)).join(' / ');
}
function varietyUseDisplay(v){return varietyDisplayText(varietyUseLabel(v)) || msg('用途は','Uses can be checked in details','用途可在詳情確認','用途可在详情确认');}
function varietyFeatureDisplay(v){return varietyDisplayText(varietyFeatureLabel(v)) || msg('特徴は詳細データで。','Check detailed data for traits.','可在詳細資料確認特徵。','可在详细资料确认特征。');}
...
function varietyCleanDisplayM71(x){
  let s=cleanPublicText(x);
  if(!s) return '';
  if(/用途出典|追加確認|現時点||が||要確認|||断定表示|表示優先|出典は|確認のため|香りメモ|詳細で確認|扱い/.test(s)) return '';
  return s;
}
function varietyRegionLabel(v){
  const prof=varietyProfile(v||{}); if(prof&&prof.region) return prof.region;
  ...
  const prefs=varietyPrefectureFromTextM71(prod,origin,field(v,'notes'),field(v,'source_urls'),field(v,'tags'),field(v,'出典s'));
  if(country==='日本'){
    if(prod && prod!=='日本') return `日本 / ${prod}`;
    if(prefs.length) return `日本 / ${prefs.slice(0,2).join('・')}`;
    return '日本';
  ...
  if(prod && prod!==country) return `${country} / ${prod}`;
  return country||msg('地域は出典欄で','Region can be checked in sources','地區可在出處欄確認','地区可在出处栏确认');
}
function varietyUseLabel(v){
  const prof=varietyProfile(v||{}); if(prof&&prof.use) return prof.use;
...
function varietyFeatureLabel(v){
  ...
  if(type) return `${type}の米品種です。食味や炊き方の細部は、出典欄の情報と合わせて。`;
  return '産地、粒形、食味、用途を出典欄と合わせて確認できる米品種です。';
}
function varietyReadableSummary(v){
  if(!v) return '米品種図鑑で産地、特徴、用途を。';
  const prof=varietyProfile(v||{}); if(prof&&prof.summary) return prof.summary;
  const name=varietyBaseName(v);
  const region=varietyRegionLabel(v);
  const type=varietyTypeGrainTextM71(v);
  const feature=varietyFeatureLabel(v);
  const use=varietyUseLabel(v);
  const head = region ? `${name}は、${region}で知られる米品種です。` : `${name}は、米品種図鑑にされている米品種です。`;
  const typeLine = type ? `${type}として。`.replace('として','の品種です') : '';
...
function conditionSummary(){
  return {title:t('waterRice'), body:msg('選んだ地域の水質情報を見ながら、硬度・pH・残留塩素と炊飯の関係を。数値が見つからない地域では、参考リンクから。最後は、その水で炊く時の硬さ・粘り・香り・水加減の見方へつなげます。','Check hardness, pH, residual chlorine, and how they relate to rice cooking for the selected area. If local numbers are not available, reference links are shown. The final view connects the water to firmness, stickiness, aroma, and water adjustment.','參考所選地區的水質資訊，確認硬度、pH、餘氯與炊飯的關係。沒有數值時會顯示參考連結。最後連到用這種水煮飯時的硬度、黏性、香氣與水量調整。','参考所选地区的水质信息，确认硬度、pH、余氯与煮饭的关系。没有数值时会显示参考链接。最后连接到用这种水煮饭时的硬度、黏性、香气与水量调整。')};
}
...
function futureBody(x){return tx(x,'body')||field(x,'body_ja','summary_ja','text_ja')||msg('本文は詳細ページで','Check the body on the detail page','本文請在詳情頁確認','正文请在详情页确认');}
...
function normalizeView(v){
  v=String(v||'home').replace(/^#/,'').trim();
  const aliases={dictionary:'varieties',glossary:'words',terms:'words',word:'words',story:'future',stories:'future',futureStory:'future',worldStories:'future',condition:'check',confirm:'check',water:'check',storage:'check',encyclopedia:'varieties',variety:'varieties',riceVarieties:'varieties',ranking:'rankings'};
  v=aliases[v]||v;
  return RN_VALID_VIEWS.includes(v)?v:'home';
}
function goView(v){switchView(normalizeView(v));return false;}

function openLearnAll(){
  S.filters.learnBatch='';
  S.filters.learn='';
  switchView('learn');
  return false;
}
function openLearnBatch(no){
  S.filters.learnBatch=String(no||'');
  S.filters.learn='';
  switchView('learn');
  return false;
}
...
async function load(){
  ...
  const res=await fetch('data/rice_navi_data.json?v=textrecovery29-googleui16-auditfix-20260701',{cache:'no-store'});
  ...
  document.querySelectorAll('.bottomnav button').forEach(b=>b.onclick=()=>{
    ...
    const v=b.dataset.view||b.dataset.go||'home';
    if(v==='learn') return openLearnAll();
    return switchView(v);
  });
...
function render(){if(!S.data)return; updateStaticI18n(); const map={home:renderHome,learn:renderLearn,literature:renderLiterature,varieties:renderVarieties,check:renderCheck,future:renderFuture,rankings:renderRankings,words:renderWords}; try{(map[S.view]||renderHome)();}catch(e){console.error('render failed',S.view,e); const target=document.getElementById(S.view)||document.getElementById('home'); if(target){target.innerHTML='<div class="card empty"><h1>表示できませんでした</h1><button onclick="switchView(\'home\')">ホーム</button></div>';}}}
function setFilter(k,v,view){S.filters[k]=v;if(view)switchView(view);else render();}

function runHomeSearch(q){
  q=String(q||'').trim();
  if(!q){switchView('home');return false;}
  const vars=S.data?.rice_varieties||[];
  const gloss=S.data?.glossary||[];
  const stories=storyList();
  const future=S.data?.future_rice||[];
  const contains=(x)=>String(x||'').toLowerCase().includes(q.toLowerCase());
  const vHit=vars.some(v=>contains(titleOfVariety(v))||contains(safeVarietyRegionJa(v))||contains(safeVarietyUseJa(v)));
  if(vHit){setVarietySearch(q);return false;}
  const gHit=gloss.some(x=>contains(termName(x))||contains(field(x,'desc_ja','related_terms_ja','category_ja')));
  if(gHit){setGlossarySearch(q);return false;}
  const fHit=future.some(x=>contains(tx(x,'title'))||contains(tx(x,'subtitle'))||contains(tx(x,'body'))||contains(field(x,'category')));
  if(fHit){S.filters.futureFocus='future';S.filters.futureq=q;S.filters.storyq='';switchView('future');return false;}
  const sHit=stories.some(x=>contains(storyTitle(x))||contains(storyCountry(x))||contains(storyBody(x,2400)));
  if(sHit){S.filters.futureFocus='stories';S.filters.storyq=q;S.filters.futureq='';switchView('future');return false;}
  setGlossarySearch(q);
  return false;
}
function openWorldStories(){S.filters.futureFocus='stories';S.filters.storyq='';S.filters.futureq='';switchView('future');return false;}
function openFutureRice(){S.filters.futureFocus='future';S.filters.futureq='';S.filters.storyq='';switchView('future');return false;}

function renderHome(){
 const d=S.data, counts=d.counts||{}, variety=pick(d.rice_varieties,'variety'), story=todayStory();
 const vf=varietyShort(variety||{});
 const word=dailyRiceWord();
 const ft=dailyFortune();
 const qPlaceholder=msg('用語・品種・物語を検索','Search rice terms, varieties, stories','搜尋用語、品種、故事','搜索术语、品种、故事');
 $('#home').innerHTML=`<div class="google-home">
   <section class="search-hero" aria-label="RICE NAVI home">
     <div class="home-logo">RICE NAVI</div>
     <div class="home-search" role="search">
       <span aria-hidden="true">⌕</span>
       <input type="search" placeholder="${esc(qPlaceholder)}" onkeydown="if(event.key==='Enter'){runHomeSearch(this.value);}">
     </div>
     <h2 class="home-actions-title">${msg('よく使う','Quick links','常用','常用')}</h2>
     <div class="home-actions home-action-cards compact-actions">
       <button type="button" onclick="switchView('varieties')">${msg('品種','Varieties','品種','品种')}</button>
       <button type="button" onclick="switchView('words')">${msg('用語','Glossary','用語','术语')}</button>
       <button type="button" onclick="switchView('future')">${msg('物語','Stories','故事','故事')}</button>
       <button type="button" onclick="S.filters.futureTab='future';switchView('future')">${msg('未来','Future','未來','未来')}</button>
       <button type="button" onclick="switchView('literature')">${msg('文献','Refs','文獻','文献')}</button>
       <button type="button" onclick="openRankingsAll()">${msg('順位','Rankings','排行','排行')}</button>
       <button type="button" onclick="switchView('check')">${msg('水質','Water','水質','水质')}</button>
       <button type="button" onclick="openLearnAll()">${msg('学ぶ','Learn','學習','学习')}</button>
     </div>
   </section>

   <section class="plain-section today-section">
     <h2>${msg('今日','Today','今日','今天')}</h2>
     <div class="simple-list">
       <button type="button" class="simple-row" onclick="switchView('future')"><span>${t('worldStories')}</span><b>${esc(storyTitle(story||{}))}</b></button>
       ${word?`<button type="button" class="simple-row" onclick="setGlossarySearch('${esc(word.title||word.meaning||'')}')"><span>${t('todayRiceWord')}</span><b>${esc(word.title)}</b></button>`:''}
       ${ft?`<button type="button" class="simple-row" onclick="setFortuneSearch('${esc(ft.process||ft.rice||ft.title)}')"><span>${t('todayFortune')}</span><b>${esc(ft.title)}</b></button>`:''}
       <button type="button" class="simple-row" onclick="setVarietySearch('${esc(vf.title)}')"><span>${t('todayRice')}</span><b>${esc(vf.title)}</b></button>
     </div>
   </section>

   <section class="plain-section story-preview">
     <h2>${esc(storyTitle(story||{}))}</h2>
     <p class="story-readable story-excerpt">${esc(storyBody(story||{},2400)).split('\n').join('<br>')}</p>
     <button type="button" class="inline-open" onclick="const p=this.previousElementSibling;p.classList.remove('story-excerpt');this.remove();">${msg('開く','Open','展開','展开')}</button>
   </section>

   <section class="plain-section">
     <h2>${msg('探す','Search','搜尋','搜索')}</h2>
     <div class="link-list">
       ${plainLink('varieties',msg('品種','Varieties','品種','品种'))}
       ${plainLink('words',msg('用語','Glossary','用語','术语'))}
       <button type="button" class="plain-link" onclick="openWorldStories()"><span>${msg('物語','Stories','故事','故事')}</span><em>›</em></button>
       <button type="button" class="plain-link" onclick="openFutureRice()"><span>${msg('未来','Future','未來','未来')}</span><em>›</em></button>
       ${plainLink('literature',msg('文献','References','文獻','文献'))}
       ${plainLink('rankings',msg('順位','Rankings','排行','排行'))}
       ${plainLink('check',msg('水質','Water quality','水質','水质'))}
       ${plainLink('learn',t('riceStudy'))}
     </div>
   </section>
 </div>`;
}

function plainLink(view,label){const action=view==='learn'?`openLearnAll()`:view==='rankings'?`openRankingsAll()`:`switchView('${view}')`;return `<button type="button" class="plain-link" onclick="${action}"><span>${esc(label)}</span><em>›</em></button>`;}
function openRankingsAll(){S.filters.rankq=''; switchView('rankings');}
...
function tempMapHTML(){
 const events=[
  ...
  [msg('糊化','Gelatinization','糊化','糊化'),msg('硬さ、芯残り、べたつきの分岐を見る。','Check how firmness, uncooked core, and stickiness branch from this point.','確認硬度、夾生與黏膩感的分歧。','确认硬度、夹生与黏腻感的分歧。'),'糊化 硬さ','words'],
  ...
  [msg('ほぐし・保温','Loosening and holding','翻鬆・保溫','翻松・保温'),msg('余分な蒸気、付着、老化、保温臭を見る。','Check excess steam, sticking, staling, and holding odor.','確認多餘蒸氣、沾黏、老化與保溫氣味。','确认多余蒸气、粘连、老化与保温气味。'),'ほぐし 老化','words']
 ];
 return `<div class="section-title"><h2>${msg('温度×時間マップ','Temperature × time map','溫度×時間地圖','温度×时间地图')}</h2></div>${homeArt('cooking_guide','card-visual','炊飯工程')}<div class="timeline">${events.map(e=>`<div class="step"><b>${esc(e[0])}</b><p>${esc(e[1])}</p><span>${esc(e[2])}</span><button class="btn secondary" onclick="${e[3]==='learn'?'openLearnAll()':`switchView('${e[3]}')`}">${msg('関連を見る','See related items','查看相關','查看相关')}</button></div>`).join('')}</div>`;
}
function renderLearn(){
 const data=(S.data.learning_basics&&S.data.learning_basics.length?S.data.learning_basics:[])||[];
 const q=S.filters.learn||'';
 const batch=String(S.filters.learnBatch||'');
 const byBatch=data.filter(x=>!batch||String(x.batch_no||'')===batch);
 let filtered=byBatch.filter(x=>!q||JSON.stringify(x).includes(q));
 if(!filtered.length && (q||batch)){ filtered=data.slice(); }
 const batches=[
  {no:'1',label:'1-25',title:'基本'},
  {no:'2',label:'26-50',title:'調整'},
  {no:'3',label:'51-75',title:'加熱'},
  {no:'4',label:'76-100',title:'現場'},
  {no:'5',label:'101-125',title:'応用'}
 ];
 const current=batches.find(b=>b.no===batch);
 const termTop=[
  ['品種','品種'],['洗米','洗米'],['浸漬','浸漬'],['水加減','水加減'],['粘り','粘り'],['粒立ち','粒立ち'],['香り','香り'],['長粒','長粒'],['保存','保存'],['加熱','加熱'],['蒸らし','蒸らし'],['水質','水質']
 ];
 const card=x=>`<div class="item learn-item"><h3>${esc(lc(x,'title')||'')}</h3>${lc(x,'short')?`<p>${esc(lc(x,'short'))}</p>`:''}<details><summary>開く</summary>${lc(x,'easy')?`<p>${esc(lc(x,'easy'))}</p>`:''}${lc(x,'field_check_points')?`<p><b>見るところ：</b>${esc(lc(x,'field_check_points'))}</p>`:''}${lc(x,'warning')?`<p><b>注意：</b>${esc(lc(x,'warning'))}</p>`:''}${lc(x,'related_terms')?`<p class="muted">${esc(lc(x,'related_terms'))}</p>`:''}</details></div>`;
 $('#learn').innerHTML=`<div class="hero"><h1>${t('riceStudy')}</h1></div>
 <div class="priority-row learn-batches"><button onclick="S.filters.learnBatch='';S.filters.learn='';renderLearn()" class="${!batch?'active':''}">すべて</button>${batches.map(b=>`<button onclick="S.filters.learnBatch='${b.no}';S.filters.learn='';renderLearn()" class="${batch===b.no?'active':''}">${b.label} ${b.title}</button>`).join('')}</div>
 <div class="toolbar"><input placeholder="探す" value="${esc(q)}" oninput="setFilter('learn',this.value)"></div>
 <div class="priority-row">${termTop.map(([key,label])=>`<button onclick="S.filters.learn='${key}';renderLearn()">${label}</button>`).join('')}</div>
 ${current?`<div class="section-title"><h2>${current.label} ${current.title}</h2></div>`:''}
 <div class="list learn-list">${filtered.map(card).join('')}</div>`;
}
...
function renderLiterature(){
 ...
 const detailHTML=detail?`<div class="card detail-card visual-card">${homeArt('research_notes','card-visual','文献要点')}<h2>${esc(literatureFieldText(detail,'title',msg('文献名','reference title','文獻名稱','文献名称'))||msg('文献名は詳細で確認','Check the reference title in details','文獻名稱請在詳情確認','文献名称请在详情确认'))}</h2><p>${chips([literatureCategoryLabel(detail.category),evidenceLabel(detail._class)])}</p><p class="small">${esc(statusInfo(detail._class))}</p><p class="data-note">${msg('要点、条件、出典を。','You can check reference titles, key points, conditions, numbers, and sources when needed.','需要時可確認文獻名稱、要點、條件、數值與出處。','需要时可确认文献名称、要点、条件、数值与出处。')}</p><p class="plain-box"><b>${msg('要点：','What this reference can confirm: ','此文獻可確認的內容：','此文献可确认的内容：')}</b>${msg('要点、条件、出典を。','You can check the key point, conditions, numbers, and sources. When related learning exists, you can also check its sources from here.','可以確認文獻要點、條件・數值與出處。若有相關學習，也可從這裡確認出處。','可以确认文献要点、条件・数值与出处。若有相关学习，也可从这里确认出处。')}</p><p><b>${msg('文献要点：','Reference key point: ','文獻要點：','文献要点：')}</b>${esc(literatureFieldText(detail,'summary',msg('要点','key point','要點','要点'))&&!same(literatureFieldText(detail,'summary',msg('要点','key point','要點','要点')),literatureFieldText(detail,'title',msg('文献名','reference title','文獻名稱','文献名称')))?literatureFieldText(detail,'summary',msg('要点','key point','要點','要点')):msg('要点を見る','Check the key point in reference details or on the source page','要點請在文獻詳情或出處頁面確認','要点请在文献详情或出处页面确认'))}</p><p><b>${msg('条件・数値：','Conditions / numbers: ','條件・數值：','条件・数值：')}</b>${esc(literatureFieldText(detail,'condition',msg('条件・数値','conditions and numbers','條件・數值','条件・数值'))||msg('条件・数値を見る','Check conditions and numbers in reference details or on the source page','條件與數值請在文獻詳情或出處頁面確認','条件与数值请在文献详情或出处页面确认'))}</p><p><b>${msg('見るポイント：','Check point: ','確認重點：','确认重点：')}</b>${esc(field(detail,'trace')?traceText(detail.trace):msg('要点、条件、出典を見ます','Use the key point, conditions, and sources to see what this reference can confirm.','從要點、條件與出處確認此文獻可確認什麼。','从要点、条件与出处确认此文献可确认什么。'))}</p>${claimDetails(field(detail,'出典s'))}${sourceDetails(field(detail,'出典s'))}<div class="priority-row"><button class="btn secondary" onclick="openLearnAll()">${msg('関連する勉強を見る','View related learning','查看相關學習','查看相关学习')}</button><button class="btn secondary" onclick="S.filters.wordq='${esc((detail.category||detail.title||'').split(/[・/ ]/)[0])}';switchView('words')">${msg('辞典で見る','View in dictionary','在辭典查看','在词典查看')}</button><button class="btn secondary" onclick="S.filters.litDetail='';renderLiterature()">${msg('詳細を閉じる','Close details','關閉詳情','关闭详情')}</button></div></div>`:'';
 $('#literature').innerHTML=`<div class="hero"><h1>${t('literatureLibrary')}</h1></div>
 <div class="countbar">${stat(classCounts.A,msg('条件まで見られる資料','References with conditions','可看條件的資料','可看条件的资料'),t('literature'))}${stat(classCounts.B,msg('要点がある資料','References with key points','有要點的資料','有要点的资料'),t('literature'))}${stat(classCounts.C,msg('出典・要点を詳細で見る資料','References needing detail check','需在詳情確認出處・要點的資料','需在详情确认出处・要点的资料'),t('literature'))}${stat(notes.length,msg('実用メモ','Practical notes','實用備註','实用备注'),msg('補足','Supplement','補充','补充'))}</div>
 <div class="toolbar"><input placeholder="${msg('文献','Search references','搜尋文獻','搜索文献')}" value="${esc(q)}" oninput="setFilter('litq',this.value)"><select onchange="setFilter('litcat',this.value)"><option value="">${ui('allCategories')}</option>${categories.map(c=>`<option ${c===cat?'selected':''}>${esc(c)}</option>`).join('')}</select><select aria-label="${msg('出典確認ステータス','Evidence status','根據確認狀態','依据确认状态')}" onchange="setFilter('evidence',this.value)"><option value="AB" ${ev==='AB'?'selected':''}>${msg('読みやすい資料中心','Readable references first','以易讀資料為主','以易读资料为主')}</option><option value="A" ${ev==='A'?'selected':''}>${msg('条件まで見られる資料','References with conditions','可看條件的資料','可看条件的资料')}</option><option value="B" ${ev==='B'?'selected':''}>${msg('要点がある資料','References with key points','有要點的資料','有要点的资料')}</option><option value="C" ${ev==='C'?'selected':''}>${msg('出典・要点を詳細で見る資料','References needing detail check','需在詳情確認出處・要點的資料','需在详情确认出处・要点的资料')}</option><option value="ALL" ${ev==='ALL'?'selected':''}>${msg('全件','All','全部','全部')}</option></select></div>
 ...
 ${detailHTML}
 <div class="countbar">${stat(filtered.length,msg('表示','Reference titles shown','顯示中的文獻標題','显示中的文献标题'))}${stat(cards.length,msg('総数','Total reference cards','文獻卡總數','文献卡总数'))}${stat(notes.length,msg('実用メモ','Practical notes','實用備註','实用备注'))}</div>
 <div class="list title-list">${filtered.map(x=>{const idx=cards.indexOf(x);const cls=x._class;return `<div class="item evidence${cls} compact-title-item"><h3>${esc(literatureFieldText(x,'title',msg('文献名','reference title','文獻名稱','文献名称'))||msg('文献名は詳細で確認','Check the reference title in details','文獻名稱請在詳情確認','文献名称请在详情确认'))}</h3><p>${chips([literatureCategoryLabel(x.category),evidenceLabel(cls)])}</p><button class="btn secondary" onclick="setLitDetail(${idx})">${msg('詳しく見る','View details','查看詳情','查看详情')}</button></div>`}).join('')}</div>
 <details class="field-note"><summary>${msg('メモを見る','View field notes','查看現場備註','查看现场备注')}</summary><p class="small">${msg('メモは、作業中の気づきや確認事項をまとめた補足です。資料の要点とは区別して読みます。','Field notes are supplementary notes from work observations and checks. Read them separately from reference key points.','現場備註是整理作業中的發現與確認事項的補充。請與資料要點區分閱讀。','现场备注是整理作业中的发现与确认事项的补充。请与资料要点区分阅读。')}</p><div class="list">${notes.map(n=>`<div class="item field-item"><h3>${esc(field(n,'title')||msg('メモ名は詳細で確認','Check note title in details','備忘名稱請在詳情確認','备注名称请在详情确认'))}</h3><p>${chips([n.category,traceText(n.decision),msg('補足メモ','Supplementary note','補充備註','补充备注')])}</p><p>${esc(field(n,'summary')||'')}</p><p><b>${msg('確認内容：','Check item: ','確認內容：','确认内容：')}</b>${esc(field(n,'condition')||msg('','Check in details','可在詳情確認','可在详情确认'))}</p><p><b>${msg('次の確認：','Next check: ','下一步確認：','下一步确认：')}</b>${esc(field(n,'next_action')||msg('','Check in details','可在詳情確認','可在详情确认'))}</p></div>`).join('')}</div></details>`;
}

// GOOGLEUI12_LEARN_REPAIR: removed legacy generated variety renderer.
// Variety cards must use protected body fields and direct in-card references only.
function renderCheck(){
 const raw=S.data.water_quality||S.data.water||{}, regions=raw.regions||[], selected=Math.max(0,Math.min(regions.length-1,Number(S.filters.waterRegion||0)||0)), region=regions[selected]||regions[0]||{}, w=raw;
 $('#check').innerHTML=`<div class="hero"><h1>${t('check')}</h1></div>
 ${card(t('waterRice'),`${waterRegionPickerHTML(regions,selected)}${waterTableHTML(region,w)}${waterIntroHTML(region,w)}`)}`;
}

function futureRiceTitle(x){return tx(x,'title')||field(x,'title_ja','future_id')||'';}
function futureRiceSubtitle(x){return tx(x,'subtitle')||field(x,'subtitle_ja')||'';}
function futureRiceBody(x){return tx(x,'body')||field(x,'body_ja')||'';}
function renderFuture(){
 const stories=storyList(), story=todayStory(), storyQ=S.filters.storyq||'', storyCountryFilter=S.filters.storyCountry||'';
 const futures=Array.isArray(S.data?.future_rice)?S.data.future_rice:[], futureQ=S.filters.futureq||'', futureCat=S.filters.futureCat||'';
 const focus=S.filters.futureFocus||'';
 ...
 const futureCats=[...new Set(futures.map(x=>field(x,'category')).filter(has))].sort();
 const futureFiltered=futures.filter(x=>(!futureQ||JSON.stringify(x).includes(futureQ))&&(!futureCat||field(x,'category')===futureCat));
 const todayIdx=todayStoryIndex();
 const todayStoryHTML=story?`<div class="card story-card today-story-pick"><h2>${msg('今日の物語','Today’s story','今日故事','今日故事')}</h2><p>${chips([storyDayDateChip(story),storyCountry(story),field(story,'region')])}</p><h3>${esc(storyTitle(story))}</h3><p><b>${esc(storySubtitle(story))}</b></p><p>${esc(storyBody(story,260)).split('\n').join('<br>')}</p><details class="story-inline-detail"><summary>${msg('全文','Full text','全文','全文')}</summary><p>${esc(storyBody(story,2400)).split('\n').join('<br>')}</p></details></div>`:'';
 const storyRows=storyFiltered.map(x=>{const idx=stories.indexOf(x); const isToday=idx===todayIdx; return `<details class="story-accordion-row${isToday?' is-today':''}"><summary><span class="story-date">${esc(storyDateLabel(x))}</span><span class="story-title-main"><b>${esc(storyTitle(x))}</b><em>${esc(storyCountry(x))}${field(x,'theme')?' / '+esc(field(x,'theme')):''}</em></span><span class="story-open">${msg('詳細','Details','詳情','详情')}</span></summary><div class="story-accordion-body"><p><b>${esc(storySubtitle(x))}</b></p><p>${esc(storyBody(x,2400)).split('\n').join('<br>')}</p><p>${chips([field(x,'texture_keywords'),field(x,'related_terms'),field(x,'related_card_search')])}</p></div></details>`}).join('');
 const futureRows=futureFiltered.map(x=>`<details class="story-accordion-row future-rice-row"><summary><span class="story-date">${esc(field(x,'category'))}</span><span class="story-title-main"><b>${esc(futureRiceTitle(x))}</b><em>${esc(futureRiceSubtitle(x))}</em></span><span class="story-open">${msg('詳細','Details','詳情','详情')}</span></summary><div class="story-accordion-body"><p>${esc(futureRiceBody(x)).split('\n').join('<br>')}</p>${field(x,'source_url')?`<p class="source">${esc(field(x,'source_url'))}</p>`:''}</div></details>`).join('');
 const futureSection=`<div class="section-title"><h2>${ui('future50')}</h2></div>
 <div class="toolbar"><input placeholder="${ui('futureSearch')}" value="${esc(futureQ)}" oninput="S.filters.futureq=this.value;renderFuture()"><select onchange="S.filters.futureCat=this.value;renderFuture()"><option value="">${ui('allCategories')}</option>${futureCats.map(c=>`<option ${c===futureCat?'selected':''}>${esc(c)}</option>`).join('')}</select></div>
 <div class="countbar">${stat(futureFiltered.length,ui('showing'))}${stat(futures.length,ui('futureTotal'))}${stat(futureCats.length,ui('category'))}</div>
 <div class="story-title-list future-rice-list">${futureRows}</div>`;
 const storySection=`${todayStoryHTML}
 ...
 $('#future').innerHTML=`<div class="hero"><h1>${msg('物語・未来','Stories / Future','故事・未來','故事・未来')}</h1></div>${focus==='future'?futureSection+storySection:storySection+futureSection}`;
}

function renderRankings(){
 const rankingRoot=S.data?.rankings||{};
 const defs=Array.isArray(rankingRoot.rankings)?rankingRoot.rankings:[];
 const rows=Array.isArray(S.data?.ranking_items_template)?S.data.ranking_items_template:[];
 const q=norm(S.filters.rankq||'').toLowerCase();
 const rankTitle=def=>tx(def,'display_title')||tx(def,'ranking_name')||field(def,'ranking_id')||'';
 const rankLead=def=>tx(def,'value')||tx(def,'short_desc')||'';
 const rankItemName=x=>tx(x,'country_or_item')||field(x,'country_or_item_ja','country_or_item_en')||'';
 const rankNote=x=>localizedValue(x,'app_note')||tx(x,'app_note')||field(x,'app_note_ja','app_note_en')||'';
 const sourceName=def=>field(def,'source_name')||'';
 const collectItems=def=>{
   const own=Array.isArray(def.items)?def.items:[];
   const linked=rows.filter(x=>field(x,'ranking_id')===field(def,'ranking_id')&&has(rankItemName(x))&&has(field(x,'value')));
   const seen=new Set();
   return own.concat(linked).filter(x=>{
     const key=[field(x,'ranking_id'),field(x,'rank'),rankItemName(x),field(x,'value')].join('|');
     if(seen.has(key)) return false;
     seen.add(key);
     return true;
   }).sort((a,b)=>(parseFloat(field(a,'rank'))||999)-(parseFloat(field(b,'rank'))||999));
 };
 const filtered=defs.filter(def=>{
   if(!q) return true;
   const hay=[rankTitle(def),rankLead(def),field(def,'category'),field(def,'source_name'),field(def,'unit')].join(' ').toLowerCase();
   return hay.includes(q);
 });
 const rowsHTML=filtered.map(def=>{
   const items=collectItems(def);
   const title=rankTitle(def);
   const lead=rankLead(def);
   const table=items.length?`<table class="rank-table"><thead><tr><th>${msg('順位','Rank','名次','名次')}</th><th>${msg('国・項目','Country / item','國家・項目','国家・项目')}</th><th>${msg('数値','Value','數值','数值')}</th><th>${msg('年','Year','年份','年份')}</th></tr></thead><tbody>${items.slice(0,10).map(x=>{const note=rankNote(x); const value=[field(x,'value'),field(x,'unit')||field(def,'unit')].filter(has).join(' ');return `<tr><td>${esc(field(x,'rank'))}</td><td><b>${esc(rankItemName(x))}</b>${note?`<br><span class="small">${esc(note)}</span>`:''}</td><td>${esc(value)}</td><td>${esc(field(x,'source_year')||field(def,'source_year'))}</td></tr>`;}).join('')}</tbody></table>`:`<p class="empty">${msg('表示できる順位がありません。','No ranking rows are available.','沒有可顯示的排行。','没有可显示的排行。')}</p>`;
   const ref=[sourceName(def),field(def,'source_year'),field(def,'unit')].filter(has).join(' / ');
   return `<article class="item rank-card"><h3>${esc(title)}</h3>${lead?`<p>${esc(lead)}</p>`:''}${table}${ref?`<p class="source">${esc(ref)}</p>`:''}</article>`;
 }).join('');
 $('#rankings').innerHTML=`<div class="hero"><h1>${msg('ランキング','Rankings','排行','排行')}</h1></div>
 <div class="toolbar"><input placeholder="${msg('ランキングを検索','Search rankings','搜尋排行','搜索排行')}" value="${esc(S.filters.rankq||'')}" oninput="S.filters.rankq=this.value;renderRankings()"></div>
 <div class="list">${rowsHTML || `<div class="card empty"><h3>${msg('ランキングが見つかりません','No rankings found','找不到排行','找不到排行')}</h3></div>`}</div>`;
}

// GOOGLEUI09: words/fortune view guard.
// The previous public build called fortuneList(), fortuneOf(), and fortuneProcessLink()
// from renderWords(), but those helpers were missing after UI cleanup. That caused the
// dictionary view to throw a ReferenceError and turn white when opening 今日の米言葉 / 今日の米占い.
function fortuneList(){
  const rf=S.data?.rice_fortune||{};
  if(Array.isArray(rf)) return rf;
  if(Array.isArray(rf.items)) return rf.items;
  return [];
}
function fortuneOf(x){
  if(!x) return {};
  return {
    id: field(x,'fortune_id'),
    title: tx(x,'title')||field(x,'title_ja')||field(x,'theme_key')||t('todayFortune'),
    message: tx(x,'message')||field(x,'message_ja'),
    one: tx(x,'one_word')||field(x,'one_word_ja'),
    number: field(x,'lucky_number_ja','lucky_number'),
    rice: field(x,'lucky_rice_ja','lucky_rice'),
    process: field(x,'lucky_process_ja','lucky_process'),
    category: field(x,'category'),
    related: field(x,'related_card_id')
  };
}
...
function fortuneProcessLink(process){
  if(!process) return '';
  return `<div class="priority-row"><button type="button" onclick="setGlossarySearch('${jsString(process)}')">${esc(process)}</button></div>`;
}
...
function renderWords(){
 ...
 $('#words').innerHTML=`<div class="hero"><h1>${msg('用語集','Glossary','用語集','术语集')}</h1></div>
 ...
 <div class="section-title"><h2>${ui('dailyWords100')}</h2></div><div class="toolbar"><select onchange="setFilter('wordcat',this.value)"><option value="">${ui('allCategories')}</option>${cats.map(c=>`<option ${c===cat?'selected':''}>${esc(c)}</option>`).join('')}</select></div><div class="countbar">${stat(dailyFiltered.length,ui('showingDailyWords'))}${stat(daily.length,ui('wordTotal'))}${stat(cats.length,ui('category'))}</div>
...
// GOOGLEUI12_LEARN_REPAIR: variety text restore guard.
// Do not compose public variety prose from component fields, tags, or generated English/Chinese summaries.
// The public variety body must come from the protected restored/body fields only.
function safeVarietyBodyJa(v){
  return cleanPublicText(field(v,'restored_body_ja','body_ja','app_detail_note_ja_v1_6','app_detail_note_ja','body','description'));
}
function safeVarietySummaryJa(v){
  return cleanPublicText(field(v,'public_summary_ja','summary_ja','summary'));
}
function safeVarietyUseJa(v){
  const s=cleanPublicText(field(v,'main_uses','use_ja','use_category_primary','use_category'));
  if(!s || /詳細で確認|確認|未確認|出典/.test(s)) return '';
  return s.replace(/業務用白飯/g,'白飯').replace(/白飯・主食/g,'白ごはん');
}
function safeVarietyRegionJa(v){
  const country=cleanPublicText(field(v,'country_ja','country_region_ja','origin_country_ja','country'))||'';
  const region=cleanPublicText(field(v,'restored_region_note_ja','region_ja','main_production_regions'))||'';
  if(country && region && region!==country) return `${country} / ${region}`;
  return country || region;
}
function renderVarieties(){
 const data=S.data.rice_varieties||[], q=S.filters.varq||'', c=S.filters.country||'', use=S.filters.use||'', texture=S.filters.texture||'', amy=S.filters.amy||'', protein=S.filters.protein||'';
 const countries=[...new Set(data.map(countryOfVariety).filter(has))].sort();
 const uses=[...new Set(data.map(safeVarietyUseJa).filter(has))].sort();
 const filtered=data.filter(v=>{
   const body=safeVarietyBodyJa(v), summary=safeVarietySummaryJa(v), useText=safeVarietyUseJa(v), region=safeVarietyRegionJa(v);
   const hay=[titleOfVariety(v), body, summary, useText, region].join(' ');
   return (!q||hay.includes(q))&&(!c||countryOfVariety(v)===c)&&(!use||useText.includes(use));
 });
 const quick=['コシヒカリ','ササニシキ','ヒノヒカリ','あきたこまち','台湾','タイ','ジャスミン','バスマティ'];
 $('#varieties').innerHTML=`<div class="hero"><h1>${msg('米辞典','Rice varieties','米品種','米品种')}</h1></div>
 <div class="toolbar"><input placeholder="${msg('品種名・地域・用途を検索','Search varieties, regions, or uses','搜尋品種、地區、用途','搜索品种、地区、用途')}" value="${esc(q)}" oninput="setFilter('varq',this.value)"><select onchange="setFilter('country',this.value)"><option value="">${ui('allCountries')}</option>${countries.map(x=>`<option ${x===c?'selected':''}>${esc(x)}</option>`).join('')}</select><select onchange="setFilter('use',this.value)"><option value="">${msg('用途','Use','用途','用途')}</option>${uses.slice(0,60).map(x=>`<option ${x===use?'selected':''}>${esc(x)}</option>`).join('')}</select></div>
 <div class="priority-row">${quick.map(k=>`<button onclick="S.filters.varq='${esc(k)}';S.filters.country='';S.filters.use='';renderVarieties()">${esc(k)}</button>`).join('')}<button onclick="S.filters.varq='';S.filters.country='';S.filters.use='';renderVarieties()">${ui('all')}</button></div>
 <div class="list">${filtered.slice(0,250).map(v=>{const title=titleOfVariety(v); const region=safeVarietyRegionJa(v); const useText=safeVarietyUseJa(v); const summary=safeVarietySummaryJa(v); const body=safeVarietyBodyJa(v)||summary; const src=field(v,'source_urls','source_text','sources','component_source_url'); return `<div class="item variety-card"><h3>${esc(title)}</h3><p>${chips([region,useText,summary])}</p>${body?`<p class="variety-summary">${esc(body)}</p>`:''}${src?`<details><summary>${msg('参考','Reference','參考','参考')}</summary>${friendlySourceHTML(src)}</details>`:''}</div>`}).join('')}</div>`;
}

load().catch(e=>{document.body.innerHTML='<main><div class="card empty"><h1>読み込みエラー</h1></div></main>'});


// public UI cleanup
try{
  window.switchView=switchView; window.openLearnAll=openLearnAll; window.openLearnBatch=openLearnBatch;
  window.goView=goView; window.runHomeSearch=runHomeSearch; window.openWorldStories=openWorldStories; window.openFutureRice=openFutureRice;
