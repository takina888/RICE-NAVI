...
function illust(kind){return `<span class="illust ${kind}"><i></i></span>`;}
...
async function load(){const res=await fetch('data/rice_navi_data_v41.json?v=41');S.data=await res.json();document.querySelectorAll('.bottomnav button').forEach(b=>b.onclick=()=>switchView(b.dataset.view));$('#lang').onchange=e=>{S.lang=e.target.value;render()};render();}
...
function renderHome(){
 ...
 $('#home').innerHTML=`<div class="hero hero-home"><div><h1>RICE NAVI</h1><p>お米の知恵を、今日の一杯へ。米・炊飯・水・保管を、初めてでも迷いにくいカードで確認します。</p></div><div class="hero-visual"><span></span></div></div>
