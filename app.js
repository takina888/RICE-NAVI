...
const I18N={
  ja:{tagline:'お米の知恵を、今日の一杯へ。',refresh:'更新',home:'ホーム',learn:'学ぶ',literature:'文献',varieties:'図鑑',check:'水質',future:'物語・未来',rankings:'ランキング',words:'辞典',todayMain:'今日のメイン',worldStories:'世界のライス物語',readMore:'本文を読む',dailyDiscovery:'今日の小さな発見',toDictionary:'辞典へ',todayRiceWord:'今日の米言葉',todayFortune:'今日の米占い',luckyNumber:'ラッキーナンバー',luckyRice:'ラッキー米種',luckyProcess:'ラッキー工程',todayRice:'今日のお米',todayCondition:'今日の米コンディション',riceFuture:'お米の未来',learnResearch:'探す',list:'一覧へ',worldStoriesDesc:'毎日ひとつ、世界の米文化を読む',glossary:'米辞典・用語集',glossaryDesc:'用語を探す',varietyBook:'米品種図鑑',varietyDesc:'品種を探す',ranking:'ランキング',rankingDesc:'世界の米データTOP10',waterRice:'現在地の水と炊飯',waterRiceDesc:'水質の公表情報と炊飯の見方',storage:'納米庫管理',storageDesc:'結露・湿度・虫を確認',literatureLibrary:'炊飯文献ライブラリ',literatureDesc:'文献名一覧と詳しく見る',locationHistory:'現在地のお米ヒストリー',locationHistoryDesc:'現在地の米文化へ',riceCultureMap:'米文化マップ',riceCultureMapDesc:'国から米文化を探す',riceStudy:'炊飯の勉強',riceStudyDesc:'洗米・浸漬・加熱・蒸らしを基本から学ぶ',yourRecord:'あなたの記録',recordText:'学んだことば・読んだ物語・見た品種を、画面内の各機能で続けて。',stories:'物語',varietiesShort:'品種',litShort:'文献',dataCount:'一覧',dataCountNote:'',days:'日目'},
...
async function load(){
  ...
  const res=await fetch('data/rice_navi_data.json?v=textrecovery29-googleui08-publiccopy-clean-20260701',{cache:'no-store'});
 ...
 $('#home').innerHTML=`<div class="google-home">
   <section class="search-hero" aria-label="RICE NAVI home">
     ...
     <div class="home-search" role="search">
       ...
       <input type="search" placeholder="${esc(qPlaceholder)}" onkeydown="if(event.key==='Enter'){S.filters.wordq=this.value;S.filters.varq=this.value;S.filters.storyq=this.value;switchView('words');}">
     </div>
     ...
     <div class="home-actions home-action-cards compact-actions">
       ...
       <button type="button" onclick="switchView('rankings')">${msg('順位','Rankings','排行','排行')}</button>
       <button type="button" onclick="switchView('check')">${msg('水質','Water','水質','水质')}</button>
       <button type="button" onclick="switchView('learn')">${msg('学ぶ','Learn','學習','学习')}</button>
     </div>
   </section>
   ...
   <section class="plain-section">
     ...
     <div class="link-list">
       ${plainLink('words',t('glossary'))}
       ${plainLink('varieties',t('varietyBook'))}
       ${plainLink('future',t('worldStories'))}
       ${plainLink('learn',t('riceStudy'))}
       ${plainLink('literature',t('literatureLibrary'))}
       ${plainLink('check',t('waterRice'))}
       ${plainLink('rankings',t('ranking'))}
     </div>
   </section>
 </div>`;
}

function plainLink(view,label){return `<button type="button" class="plain-link" onclick="switchView('${view}')"><span>${esc(label)}</span><em>›</em></button>`;}
...
function renderLearn(){
 ...
 <div class="section-title"><h2>${msg('基本','Basics','基礎','基础')}</h2></div>
 ...
 <div class="countbar">${stat(filtered.length,msg('表示','Lessons shown','顯示中的學習','显示中的学习'))}${stat(data.length,msg('総数','Current lessons','目前學習項目','当前学习项目'),msg('100項目を25件ずつ作成','Building 100 items in batches of 25','以每批25則建立100項','以每批25条建立100项'))}${stat((S.data.counts&&S.data.counts.learning_basics_target)||100,msg('目標','Target','目標','目标'),msg('学ぶ100項目','100 learning items','學習100項','学习100项'))}</div>
...
function renderVarieties(){
 ...
 $('#varieties').innerHTML=`<div class="hero"><h1>${t('varietyBook')}</h1>${q?`<p class="search-chip">${msg('検索中：','Searching: ','搜尋中：','搜索中：')}${esc(q)}</p>`:''}</div>
...
function renderFuture(){
 ...
 $('#future').innerHTML=`<div class="hero"><h1>${ui('worldStories')}</h1></div>
 ${todayStoryHTML}
...
function renderWords(){
 ...
 $('#words').innerHTML=`<div class="hero"><h1>${ui('riceDictionaryWords')}</h1></div>
