...
/* v78.47 early visual fallback */
function rnFallbackSvgDataV7847(label){
  label = String(label||'RICE NAVI').slice(0,32);
  return 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="640" height="420" viewBox="0 0 640 420"><rect width="640" height="420" rx="36" fill="#eaf6ff"/><rect x="26" y="26" width="588" height="368" rx="28" fill="none" stroke="#cfe6f7" stroke-width="4"/><ellipse cx="145" cy="155" rx="38" ry="62" fill="#1677c8"/><text x="64" y="292" fill="#143650" font-size="36" font-weight="800" font-family="Arial">RICE NAVI</text><text x="64" y="335" fill="#5f788c" font-size="22" font-family="Arial">'+label+'</text></svg>');
}
function rnAnyVisualSrcV7847(category, name){
  try{
    name = String(name||'').replace(/\.svg$/,'');
    if(window.rnVisualAssetSrc){
      const src = window.rnVisualAssetSrc(category, name);
      if(src && /^data:image\/svg\+xml/.test(src)) return src;
    }
  }catch(e){}
  return rnFallbackSvgDataV7847(name||category);
}


/* v78.47 preflight compatibility aliases */
window.RN_BOOT_ERRORS = [];
window.addEventListener('error', function(e){
  try{ window.RN_BOOT_ERRORS.push((e.message||'error') + ' @ ' + (e.filename||'') + ':' + (e.lineno||'')); }catch(_){}
});
function countryVisualSlugV7824(name){ try{ return countryVisualV7824(name); }catch(e){ return ''; } }
function storyVisualPathV7824(key){ try{ return (window.rnVisualAssetSrc ? window.rnVisualAssetSrc('stories', String(key||'').replace(/\.svg$/,'')) : ''); }catch(e){ return ''; } }
function countryVisualPathV7824(key){ try{ return (window.rnVisualAssetSrc ? window.rnVisualAssetSrc('countries', String(key||'').replace(/\.svg$/,'')) : ''); }catch(e){ return ''; } }
function illustrationVisualPathV7845(key){ try{ return (window.rnVisualAssetSrc ? window.rnVisualAssetSrc('illustrations', String(key||'').replace(/\.svg$/,'')) : ''); }catch(e){ return ''; } }
function riceTypeVisualPathV7845(key){ try{ return (window.rnVisualAssetSrc ? window.rnVisualAssetSrc('rice-types', String(key||'').replace(/\.svg$/,'')) : ''); }catch(e){ return ''; } }

const S={data:null,view:'home',lang:'ja',filters:{}};
const RN_ASSET_VERSION='78ui47';
const $=s=>document.querySelector(s);

/* v78.47 compatibility aliases for old visual override layers */
var countryVisualSlugV7824 = (typeof countryVisualSlugV7824==='function') ? countryVisualSlugV7824 : (typeof countryVisualV7824==='function' ? countryVisualV7824 : function(){return 'world-map-rice';});
...
function illust(kind){
  ...
  return `<span class="illust ${esc(kind)}"><img src="${rnBundledVisualV7845 ? rnBundledVisualV7845("ui-icons",name) : ""}" alt="" loading="lazy"></span>`;
}
...
async function load(){
  const home=$('#home');
  try{
    const res=await fetch('data/rice_navi_data.json?v=78ui47',{cache:'no-store'});
...
/* === v78.47 public UI cleanup: hide internal labels and make navigation buttons reliable. === */
...
/* === v78.47 sweep pass: public text scrub + safer internal navigation. === */
...
/* v78.47: start after public cleanup overrides are defined. */



/* === v78.47 relationship-link sweep: add links where links should exist, not only repair broken links. === */
...
/* v78.47: start moved by v78.47. */


/* === v78.47 iterative sweep: public wording, link presence, and readable text guard === */
...
function v787EnsureBackLinks(root=document){
  try{
    // Any article/item/card with details should have a safe related-link panel after v78.47.
...
/* v78.47: start after font/link/internal-word overrides are defined. */
/* v78.47: load call moved to final boot */
/* v78.47 learn-section cleanup
...
/* v78.47 GPS water-quality workflow
...
/* === v78.47 daily-page redesign: one daily item only; archive/search separated ===
...
/* === v78.47 archive UX: title list + tap-to-open details + back controls ===
...
/* === v78.47: homepage as attraction/navigation entry + organized UI icons === */
const RN_BUILD_V7813='v78.47 / build 78ui47';
function rnIconV7813(name){ return `<img class="rn-ui-icon-v7813" src="${rnBundledVisualV7845 ? rnBundledVisualV7845("ui-icons",pEsc(name||'home')) : ""}" alt="" loading="lazy">`; }
...
/* === v78.47: rice variety visual category continuation + cleaner public pages === */
const RN_BUILD_V7814='v78.47 / build 78ui47';
...
/* === v78.47 visual asset continuation: illustrations + fixed folder references === */
...
function storyVisualChipV7817(x){
  ...
  if(c) return `<div class="archive-visual-chip"><img src="${rnBundledVisualV7845 ? rnBundledVisualV7845("countries",c) : ""}" alt="" loading="lazy"><span>${pEsc(country)}</span></div>`;
  return `<div class="archive-visual-chip"><img src="${rnBundledVisualV7845 ? rnBundledVisualV7845("stories","rice-bowl-culture") : ""}" alt="" loading="lazy"><span>${pEsc(country||'米文化')}</span></div>`;
}
const renderHomeV7817=function(){
  ...
  $('#home').innerHTML=`<div class="home-v7813 home-v7814">
    ...
    <p class="rn-version-v7813">RICE NAVI v78.47 / build 78ui47</p>
  </div>`;
};
...
/* === v78.47 page visual continuation: header illustrations + compact page logic ===
...
/* === v78.47 visual balance + safe public display continuation ===
   Keep home as an entry/search page. Keep detail pages title-first. */
const RN_PUBLIC_BUILD_V7819='RICE NAVI v78.47 / build 78ui47';
...
/* === v78.47 semantic visual continuation: more UI icons + no-root-asset bug cleanup === */
const RN_PUBLIC_BUILD_V7823='RICE NAVI v78.47 / build 78ui47';
...
/* === v78.47 country/story visual expansion ===
...
const RN_PUBLIC_BUILD_V7824='RICE NAVI v78.47 / build 78ui47';
...
function storyVisualChipV7824(x){
  ...
  const countryImg=c?`<img src="${rnBundledVisualV7845 ? rnBundledVisualV7845("countries",c) : ""}" alt="" loading="lazy">`:'';
  return `<div class="archive-visual-chip archive-visual-chip-v7824"><img src="${rnBundledVisualV7845 ? rnBundledVisualV7845("stories",pEsc(theme)) : ""}" alt="" loading="lazy">${countryImg}<span>${pEsc(country||'米文化')}</span></div>`;
}
function historyVisualChipV7824(x){
  ...
  const img=c?`${rnBundledVisualV7845 ? rnBundledVisualV7845("countries",c) : ""}`:`${rnBundledVisualV7845 ? rnBundledVisualV7845("illustrations","history-map") : ""}`;
...
/* === v78.47 visual discovery pass: show country/story visuals only as entry points === */
const RN_PUBLIC_BUILD_V7825='RICE NAVI v78.47 / build 78ui47';
function rnVisualEntryV7825(kind,title,sub,img,onclick){
  const src=kind==='country'?`${rnBundledVisualV7845 ? rnBundledVisualV7845("countries",pEsc(img)) : ""}`:kind==='story'?`${rnBundledVisualV7845 ? rnBundledVisualV7845("stories",pEsc(img)) : ""}`:`${rnBundledVisualV7845 ? rnBundledVisualV7845("illustrations",pEsc(img)) : ""}`;
...
/* v78.47: keep homepage as a concise entry point.
...
function rnFocusEntryV7826(kind,title,sub,key,onclick){
  const src = kind==='story'
    ...
    : kind==='country'
      ...
      : rnBundledVisualV7845 ? rnBundledVisualV7845('illustrations',key) : '';
...
const RN_PUBLIC_BUILD_V7826='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47: illustration insertion continuation. Focus only on visual assets and where they appear. */
function rnAsset7827(folder,name){return `assets/${folder}/${pEsc(name)}.svg?v=78ui47`;}
...
const RN_PUBLIC_BUILD_V7827='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47: illustration insertion continuation. More story/country/rice visuals, no UI detour. */
...
const RN_PUBLIC_BUILD_V7828='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47: illustration insertion continuation only. More story/country/rice visual mappings. */
...
const countryVisualSlugV7829Base = (typeof countryVisualSlugV7824==='function'?countryVisualSlugV7824:(typeof countryVisualV7824==='function'?countryVisualV7824:function(){return 'world-map-rice';}));
...
const RN_PUBLIC_BUILD_V7829='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47: illustration continuation only. Add more cultural/rice-type mappings. */
...
const countryVisualSlugV7830Base = (typeof countryVisualSlugV7824==='function'?countryVisualSlugV7824:(typeof countryVisualV7824==='function'?countryVisualV7824:function(){return 'world-map-rice';}));
countryVisualSlugV7824=function(name){
  const s=String(name||'').toLowerCase();
...
const RN_PUBLIC_BUILD_V7830='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: more story and rice-type matching */
const RN_PUBLIC_BUILD_V7831='RICE NAVI v78.47 / build 78ui47';
...
if(_varietyVisualCardV7831){
  varietyVisualCardV7814=function(v){
    ...
    return html.replace('<article class="variety-feature-v7814">', `<article class="variety-feature-v7814"><div class="archive-visual-chip archive-visual-chip-v7824"><img src="${rnBundledVisualV7845 ? rnBundledVisualV7845("rice-types",extra) : ""}" alt="" loading="lazy"><span>${pEsc(titleOfVariety(v||{}))}</span></div>`);
  }
}
...
/* v78.47 illustration continuation: rice processing, daily cards, water/storage visuals */
const RN_PUBLIC_BUILD_V7832='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: Japanese/Taiwan/home-water and regional rice visuals */
const RN_PUBLIC_BUILD_V7833='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: regional dishes and theme header matching */
const RN_PUBLIC_BUILD_V7834='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: restored from v78.47 package, re-applied v78.47-v78.47 visual batches, then added quality/storage/factory visuals */
const RN_PUBLIC_BUILD_V7838='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: seed, cultivation, quality, traceability matching */
const RN_PUBLIC_BUILD_V7839='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: future and resilience category coverage */
const RN_PUBLIC_BUILD_V7840='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: glossary, literature, ranking, search-link coverage */
const RN_PUBLIC_BUILD_V7841='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 illustration continuation: daily, fortune, GPS, troubleshoot linkage coverage */
const RN_PUBLIC_BUILD_V7842='RICE NAVI v78.47 / build 78ui47';
...
/* v78.47 GitHub upload under-100-files mode:
   visual SVGs are bundled in js/visual_assets.js as data URIs. */
const RN_PUBLIC_BUILD_V7843='RICE NAVI v78.47 / build 78ui47 GitHub under100 load-fix';
...
/* v78.47 stable GitHub under100: force bundled visuals, avoid missing asset folders */
const RN_PUBLIC_BUILD_V7845='RICE NAVI v78.47 / build 78ui47 stable under100';

function rnBundledVisualV7845(category, name){
  try{
    name = String(name||'').replace(/\.svg$/,'');
    if(window.rnVisualAssetSrc) return window.rnVisualAssetSrc(category, name);
  }catch(e){}
  return 'data:image/svg+xml;charset=UTF-8,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22640%22 height=%22420%22 viewBox=%220 0 640 420%22%3E%3Crect width=%22640%22 height=%22420%22 rx=%2236%22 fill=%22%23eaf6ff%22/%3E%3Ctext x=%22320%22 y=%22225%22 text-anchor=%22middle%22 font-size=%2238%22 fill=%22%23143650%22 font-family=%22Arial%22 font-weight=%22700%22%3ERICE NAVI%3C/text%3E%3C/svg%3E';
}

storyVisualPathV7824=function(key){ return rnBundledVisualV7845('stories',key); };
countryVisualPathV7824=function(key){ return rnBundledVisualV7845('countries',key); };
countryVisualSlugV7824=function(name){ try{return countryVisualV7824(name);}catch(e){return '';} };

if(typeof rnAssetImgV7817 === 'function'){
  rnAssetImgV7817=function(category, name, cls){
    return `<img class="${pEsc(cls||'rn-asset-img')}" src="${rnBundledVisualV7845(category,name)}" alt="" loading="lazy">`;
  };
}

if(typeof rnIconV7813 === 'function'){
  rnIconV7813=function(name){
    const src=rnBundledVisualV7845('ui-icons', name);
    return `<img class="rn-ui-icon-v7813" src="${src}" alt="" loading="lazy">`;
  };
}

if(typeof illust === 'function'){
  illust=function(name, cls){
    const src=rnBundledVisualV7845('ui-icons', name);
    return `<img class="${pEsc(cls||'icon')}" src="${src}" alt="" loading="lazy">`;
  };
}

storyVisualChipV7824=function(x){
  const country=storyCountry(x||{});
  let c=''; try{ c=countryVisualV7824(country); }catch(e){}
  let theme='rice-bowl-culture'; try{ theme=storyThemeVisualV7824(x||{}) || theme; }catch(e){}
  const countryImg=c?`<img src="${rnBundledVisualV7845('countries',c)}" alt="" loading="lazy">`:'';
  return `<div class="archive-visual-chip archive-visual-chip-v7824"><img src="${rnBundledVisualV7845('stories',theme)}" alt="" loading="lazy">${countryImg}<span>${pEsc(country||'米文化')}</span></div>`;
};

historyVisualChipV7824=function(x){
  const name=(typeof histCountryName==='function') ? histCountryName(x||{}) : field(x,'country','country_ja');
  let c=''; try{ c=countryVisualV7824(name); }catch(e){}
  const src=c?rnBundledVisualV7845('countries',c):rnBundledVisualV7845('illustrations','history-map');
  return `<div class="archive-visual-chip archive-visual-chip-v7824"><img src="${src}" alt="" loading="lazy"><span>${pEsc(name||'地域')}</span></div>`;
};

rnVisualEntryV7825=function(kind,title,sub,img,onclick){
  const cat=kind==='country'?'countries':kind==='story'?'stories':'illustrations';
  return `<button class="visual-entry-v7825" onclick="${onclick}"><img src="${rnBundledVisualV7845(cat,img)}" alt="" loading="lazy"><span><b>${pEsc(title)}</b><small>${pEsc(sub)}</small></span></button>`;
};

rnFocusEntryV7826=function(kind,title,sub,key,onclick){
  const cat=kind==='country'?'countries':kind==='story'?'stories':'illustrations';
  return `<button class="home-focus-card-v7826" onclick="${onclick}"><img src="${rnBundledVisualV7845(cat,key)}" alt="" loading="lazy"><span><b>${pEsc(title)}</b><small>${pEsc(sub)}</small></span></button>`;
};

if(typeof varietyVisualCardV7814 === 'function'){
  const _varietyVisualCardStableV7845 = varietyVisualCardV7814;
  varietyVisualCardV7814=function(v){
    let extra=''; try{ extra = (typeof varietyVisualV7831==='function') ? varietyVisualV7831(v||{}) : ''; }catch(e){}
    const html = _varietyVisualCardStableV7845(v||{});
    if(!extra) return html;
    const src = rnBundledVisualV7845('rice-types', extra);
    return html.replace('<article class="variety-feature-v7814">', `<article class="variety-feature-v7814"><div class="archive-visual-chip archive-visual-chip-v7824"><img src="${src}" alt="" loading="lazy"><span>${pEsc(titleOfVariety(v||{}))}</span></div>`);
  };
}

const _renderHomeStableV7845=renderHome;
renderHome=function(){
  try{
    _renderHomeStableV7845();
    const v=document.querySelector('.rn-version-v7813');
    if(v) v.textContent=RN_PUBLIC_BUILD_V7845;
    rnPublicCleanV7819('home');
  }catch(e){
    const home=document.getElementById('home')||document.body;
    home.innerHTML=`<main class="container"><section class="card"><h1>RICE NAVI</h1><p>表示処理でエラーが発生しました。</p><p class="muted">${pEsc(e.message||String(e))}</p></section></main>`;
    console.error(e);
  }
};

if(typeof load === 'function'){
  const _loadStableV7845 = load;
  load = async function(){
    try{
      await _loadStableV7845();
    }catch(e){
      console.error('load failed', e);
      const home=document.getElementById('home')||document.body;
      home.innerHTML=`<main class="container"><section class="card"><h1>RICE NAVI</h1><p>読み込みまたは表示でエラーが発生しました。</p><p class="muted">${pEsc(e.message||String(e))}</p></section></main>`;
    }
  };
}

render=function(){ if(!S.data)return; const map={home:renderHome,learn:renderLearn,literature:renderLiterature,varieties:renderVarieties,check:renderCheck,future:renderFuture,rankings:renderRankings,words:renderWords,detail:renderArchiveV7810}; try{(map[S.view]||renderHome)();}catch(e){console.error('render failed', e); document.body.insertAdjacentHTML('afterbegin',`<div style="padding:12px;background:#fff3cd;color:#5c4200">表示エラー: ${pEsc(e.message||String(e))}</div>`);} };
try{ if(S.data) render(); }catch(e){ console.warn('v7845 rerender skipped', e); }




/* v78.47 robust visual + navigation fix */
const RN_PUBLIC_BUILD_V7847='RICE NAVI v78.47 / build 78ui47 visual-nav fix';

function rnBundledVisualV7847(category, name){
  return rnAnyVisualSrcV7847(category, name);
}

function rnFixBrokenImagesV7847(root=document){
  root.querySelectorAll('img').forEach(img=>{
    const src = img.getAttribute('src') || '';
    if(!src || src.startsWith('assets/') || src.includes('/assets/')){
      img.setAttribute('src', rnFallbackSvgDataV7847(img.getAttribute('alt')||'RICE NAVI'));
    }
...
function rnWireNavigationV7847(){
  try{
    document.querySelectorAll('.bottomnav button[data-view]').forEach(b=>{
      b.onclick = () => {
        try{
          ...
          setTimeout(()=>rnFixBrokenImagesV7847(document),50);
        }catch(e){ console.error('nav failed', e); }
      };
    });
  }catch(e){ console.error('wire nav failed', e); }
}

const _renderStableV7847 = render;
render=function(){
  try{
    _renderStableV7847();
  }catch(e){
    console.error('render failed v78.47', e);
    const home=document.getElementById('home')||document.body;
    home.innerHTML=`<main class="container"><section class="card"><h1>RICE NAVI</h1><p>表示処理でエラーが発生しました。</p><p class="muted">${(typeof pEsc==='function')?pEsc(e.message||String(e)):String(e.message||e)}</p></section></main>`;
  }
  rnWireNavigationV7847();
  rnFixBrokenImagesV7847(document);
  const v=document.querySelector('.rn-version-v7813');
  if(v) v.textContent=RN_PUBLIC_BUILD_V7847;
};

if(typeof rnAssetImgV7817 === 'function'){
  rnAssetImgV7817=function(category, name, cls){
    return `<img class="${pEsc(cls||'rn-asset-img')}" src="${rnBundledVisualV7847(category,name)}" alt="" loading="lazy">`;
  };
}
rnIconV7813=function(name){
  return `<img class="rn-ui-icon-v7813" src="${rnBundledVisualV7847('ui-icons',name)}" alt="" loading="lazy">`;
};
illust=function(name, cls){
  return `<img class="${pEsc(cls||'icon')}" src="${rnBundledVisualV7847('ui-icons',name)}" alt="" loading="lazy">`;
};

storyVisualPathV7824=function(key){ return rnBundledVisualV7847('stories',key); };
countryVisualPathV7824=function(key){ return rnBundledVisualV7847('countries',key); };

if(typeof storyVisualChipV7824 === 'function'){
  storyVisualChipV7824=function(x){
    const country=storyCountry(x||{});
    let c=''; try{ c=countryVisualV7824(country); }catch(e){}
    let theme='rice-bowl-culture'; try{ theme=storyThemeVisualV7824(x||{}) || theme; }catch(e){}
    const countryImg=c?`<img src="${rnBundledVisualV7847('countries',c)}" alt="" loading="lazy">`:'';
    return `<div class="archive-visual-chip archive-visual-chip-v7824"><img src="${rnBundledVisualV7847('stories',theme)}" alt="" loading="lazy">${countryImg}<span>${pEsc(country||'米文化')}</span></div>`;
  };
}
if(typeof historyVisualChipV7824 === 'function'){
  historyVisualChipV7824=function(x){
    const name=(typeof histCountryName==='function') ? histCountryName(x||{}) : field(x,'country','country_ja');
    let c=''; try{ c=countryVisualV7824(name); }catch(e){}
    return `<div class="archive-visual-chip archive-visual-chip-v7824"><img src="${rnBundledVisualV7847('countries',c||'rice-asia-general')}" alt="" loading="lazy"><span>${pEsc(name||'地域')}</span></div>`;
  };
}

document.addEventListener('click', ()=>setTimeout(()=>{rnWireNavigationV7847();rnFixBrokenImagesV7847(document);},80), true);
document.addEventListener('change', ()=>setTimeout(()=>{rnWireNavigationV7847();rnFixBrokenImagesV7847(document);},80), true);

/* v78.47 final boot: execute load only once after all overrides are defined */
const RN_PUBLIC_BUILD_V7846='RICE NAVI v78.47 / build 78ui47 visual-nav fix';
(function(){
  let started=false;
  async function rnStartV7846(){
    if(started) return;
    started=true;
    try{
      if(typeof load === 'function') await load();
      else throw new Error('load function is not defined');
    }catch(e){
      console.error('RICE NAVI final boot failed', e);
      const home=document.getElementById('home')||document.body;
      const msg=(typeof pEsc==='function') ? pEsc(e.message||String(e)) : String(e.message||e);
      home.innerHTML=`<main class="container"><section class="card"><h1>RICE NAVI</h1><p>起動処理でエラーが発生しました。</p><p class="muted">${msg}</p></section></main>`;
    }
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', rnStartV7846, {once:true});
  else rnStartV7846();
})();
